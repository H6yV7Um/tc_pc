...
(function() {
  ...
  $('#changelong thead th span').unbind('click').click(function(){
    $('#changelong thead th span').removeClass('on');
    $(this).addClass('on');
    if ($(this).attr('rel')=='long') {
      $('#changelong tbody').show();$('.bonus-list').hide();
    }else {
      $('#changelong tbody').hide();$('.bonus-list').show();
    }
  });

  ////console.log(e.getNowLotteryId(),'getNowLotteryId');
	...
	function(e) {
		...
		return e.cookie.json = !0, t.templateSettings = {
			...
			initialize: function(t) {
				...
				}, t), this.lotteryId = this.options.lotteryId, this.isOpen = !0, this.isQuickMode = 0,  this.refreshDuration = 120, this.amount = 0, this.data = [], this.records = [], this.betInfo = null, this.$doc = e(document), this.$play = e("#play"), this.PrePeriodNumber = null;
				var i = this.options.filter;//this.setQuickMode(),
				if (i) {
        ...
        ////console.log('initializeinitializeinitialize');
        ...
        ////console.log($('body .container').height());
			...
			refresh: function(a) {
				var e = this;
        //alert('111');
				// //console.log(e._renderBall)
				this.getBetInfo().done(function(t) {
          //console.log(t,'getBetInfogetBetInfogetBetInfo');
					if(t.Success){
						// t.Obj.CloseCount = (t.Obj.OpenCount - 0) + 4;
      ...
      loadbonus:function(e,_b) {
        var allbonus = [];
        var allrecent = [];

        this.getOrderRecent().done(function(oren) {
          ////console.log('getOrderRecent');
          var allre_record = oren.data.list;
          var shortno = function(id) {
            return String(id).replace(_.take(id.split(''),16).join(''),'');
          }
          allrecent.push('<li class="title"><span class="first">玩法</span><span class="no">订单号</span><span class="last">金额</span></li>');
          for (var oi = 0; oi < allre_record.length; oi++) {
            allrecent.push('<li><span class="first">'+allre_record[oi].method+'</span><span class="no">'+shortno(allre_record[oi].billno)+'</span><span class="last">'+allre_record[oi].money+'</span></li>');
          }
          $('.aside-recent ul.recent-list').html(allrecent.join(''));
        });
        //alert('loadbonus');
        //allbonus.push('<li class="blank"></li>');

        ////console.log(this.getNowLotteryId(),'getNowLotteryId');
        var pankouLink = {
          '80':'11',
          '102':'22',
          '101':'23',
          '100':'24',
          '110':'43','90':'31','85':'200','86':'201','87':'202',
          '70':'601','71':'205','72':'206','104':'23','105':'28','191':'88'
        }
        var pankoucn = {
          '104':'江西11选5','23':'江西11选5',
          '105':'江苏11选5','28':'江西11选5',
          '70':'东京1.5分彩',
          '71':'首尔1.5分彩',
          '72':'纽约1.5分彩 ',
          '80':'重庆时时彩',
          ...
          '85':'菲律宾1.5分彩',
          '86':'菲律宾2分彩',
          '87':'菲律宾5分彩','88':'韩国1.5分彩',
          '90':'江苏快3'
        }
        $('.lottery-code-trend').attr('target','_blank').attr('href','/static/lottery-trend.html?id='+this.getNowLotteryId()+'&w=1&q=50&chs='+pankoucn[this.getNowLotteryId()]);
        var panid = this.getNowLotteryId();
        if (typeof pankouLink[panid] !='undefined') {
          $('.offical').attr('target','_self').attr('href','/static/lt.html#'+pankouLink[panid]);
        }

				this.getBonusInfo().done(function(bt) {
		        if ($('#changelong .full').size()>0) {
		          $('.bonus-list').show();
		        }
	          var allbonus_record = bt;
	          ////console.log(bt.data);
	          for (var reci = 0; reci < allbonus_record.length; reci++) {
	            var thisrec = allbonus_record[reci].issue;
							var joinstr = null;
							// //console.log(allbonus_record[reci].code !== null,allbonus_record[reci].code !== 'null',allbonus_record[reci].code,allbonus_record[reci].code.toString() )
							if(allbonus_record[reci].code !== null && allbonus_record[reci].code !== 'null'){
								 joinstr = allbonus_record[reci].code.split(',').join('</i><i>');
								if (thisrec[1].length>5) {
		              joinstr = String(thisrec[1]).replace(/(\d\d)(?!\b)/g,'$1</i><i>');
		            }
								allbonus.push('<li><span class="first">'+thisrec+'</span><span class="last"><i>'+joinstr+'</i></span></li>');
							}else{
								allbonus.push('<li><span class="first">'+thisrec+'</span><span class="last">等待开奖</span></li>');
							}
	          }
	          $('.bonus-list ul.bonus-data').html(allbonus.join(''));
	          $('.lottery-open-list ul.list').html(allbonus.join(''));
						$('#prev-issue').text(allbonus_record[0].issue);
						if(allbonus_record[0].code !== null && allbonus_record[0].code !== 'null'){
							_b['PreResult'] = allbonus_record[0].code;
							_b['_ke_isopenlottery'] = true;
							return e._renderBall(_b), true;
						}
						$('#prev-bs').html("开奖中...");
						return false;
	        });
      ...
      loadma:function(e) {
        var that = this;
        var madatefilter = function(ma,lid,tag) {
          //console.log(ma);
          var topfive = [],firstag=[],qishuchk={},qishuchk_yi={},nowqishu=[],qisel=[];
          chumadict = ma.data[tag];
          //console.log(chumadict,'chumadictchumadictchumadict');
          for (var key in chumadict) {
            firstag.push(key) 
          }
          //console.log(firstag,'firstagfirstagfirstag');
          if (firstag.length==0) {
            //alert('1');
            $('.lottery-ma .ma-lst ul').html('<li class="ma-blank"><div class="ma-inner"><span>暂无数据</span></div></li>');
            return;  
          }
          ...
          for (var k = 3; k < firstag.length; k++) {
            liangm = Object.assign(liangm, chumadict[firstag[k]]);  
          }
          //console.log(liangm,'liangmliangmliangmliangmliangmliangm');
          var liangmsorted = Object.keys(liangm).sort(function(a,b){return liangm[b]-liangm[a]});
          //console.log(liangmsorted,'liangmsorted');
          if (typeof liangm !='undefined') {
            for (var si = 0; si < liangmsorted.length; si++) {
            //for (var lkey in liangmsorted) {
              //console.log(t.topfive.length);
              var lkey = liangmsorted[si];
              if (topfive.length<99) {
                topfive.push('<li><div class="ma-inner"><span rel="'+lkey.split('@')[0]+'">'+lkey.replace('@','<i>@')+'</i></span><span class="last">'+liangm[lkey]+'期</span></div></li>')
                //t.topfive.push({'name':lkey,'time':liangm[lkey]})
                if (typeof qishuchk[liangm[lkey]] =='undefined') {
                  qishuchk[liangm[lkey]] = ['<li><div class="ma-inner"><span rel="'+lkey.split('@')[0]+'">'+lkey.replace('@','<i>@')+'</i></span><span class="last">'+liangm[lkey]+'期</span></div></li>'];
                  nowqishu.push(parseInt(liangm[lkey],10));
                }else {
                  qishuchk[liangm[lkey]].push('<li><div class="ma-inner"><span rel="'+lkey.split('@')[0]+'">'+lkey.replace('@','<i>@')+'</i></span><span class="last">'+liangm[lkey]+'期</span></div></li>');
                }
              }
            }
            ...
            $('.lottery-ma .ma-lst ul').html(topfive.join(''));
          }else {
            $('.lottery-ma .ma-lst ul').html('<li class="ma-blank"><div class="ma-inner"><span>暂无数据</span></div></li>');
          }
          //console.log(qishuchk,'qishuchkqishuchkqishuchk');
          //console.log(nowqishu);
          var sortedqishu = nowqishu.sort(function(a,b){return a-b}).reverse();
          //console.log(sortedqishu);
          for (var j = 0; j < sortedqishu.length; j++) {
            qisel.push('<option value="'+sortedqishu[j]+'" '+(j==0 ? 'selected="selected"' :'')+'>'+sortedqishu[j]+'期</option>');  
          }
          //console.log(qisel,'qiselqisel');
          $('.lottery-ma #ma-times').html(qisel.join(''));

          $('.lottery-ma #ma-times').on('change',function() {
            //console.log($(this).val(),qishuchk[$(this).val()].join(''));
            if (typeof qishuchk[$(this).val()] !='undefined') {
              $('.lottery-ma .ma-lst ul').html(qishuchk[$(this).val()].join(''));  
            }
          })
          //t.allcns = t.getCnName;
        }
        var lid = parseInt(this.getNowLotteryId(),10);
        var nowtag = $('.lottery-ma .ma-title .on').attr('rel');
        //alert(lid);
        this.getMaMap();

        this.getMaData().done(function(ma) {
          //console.log(ma,'loadma');
          $('.lottery-ma').data('m',ma);
          madatefilter(ma,lid,nowtag);
          
          $('.lottery-ma .ma-title a').off('click').on('click',function() {
            var that = $(this);
            $('.lottery-ma .ma-title .on').removeClass('on');
            that.addClass('on');
            var nowtag = $('.lottery-ma .ma-title .on').attr('rel');
            //console.log(nowtag,$('.lottery-ma').data('m'));
            madatefilter($('.lottery-ma').data('m'),lid,nowtag);
          });

          $('.lottery-ma .ma-lst').off('click').on('click','li',function() {
            var nowQuickDat = [];
            var nowQuickText = String($(this).find('.ma-inner span').first().attr('rel')).replace('大小','').replace('冠亚和值','冠亚和').replace('单双','');
            if (lid=='110') {
              nowQuickText = nowQuickText.replace('球','名');
            }
            var nowQuickBall = String($(this).find('.ma-inner i').text()).replace('@','');
            var nowRate = '1.960';
            //console.log(nowQuickText,nowQuickBall);
            var jid = '-1';
            //console.log(that.nowMaMap,''+nowQuickText+'_'+nowQuickBall,'11111111111');
            if (typeof that.nowMaMap[''+nowQuickText+'_'+nowQuickBall] !='undefined') {
              //console.log(that.nowMaMap[''+nowQuickText+'_'+nowQuickBall]);
              var ratejid = that.nowMaMap[''+nowQuickText+'_'+nowQuickBall];
              nowRate = that.allRateMap[ratejid];
              //console.log(ratejid);
              jid = ratejid;
              //console.log(that.allRateMap[ratejid],ratejid);
            }else {
              console.log(that.nowMaMap,nowQuickText+'_'+nowQuickBall);  
            }
            //e.ui.msg('111')  
            ...
            that.openQuickDialog(function() {
              
              nowQuickDat.push({
                'BetContext':(String(nowQuickText).indexOf('冠亚')>-1 ? '冠亚'+nowQuickBall : nowQuickBall),
                'BetType':1,
                'IsForNumber':false,
                'IsTeMa':false,
                'Lines':nowRate,
                'Money':Number($('#quick_money').val()).toFixed(2),
                'gname':nowQuickText,
                'id':jid,
                'mingxi_1':0
              });
              
              e.bet(e.lotteryId, nowQuickDat).done(function(t) {
                e.handlerResult(t);
              });
              //e.bet(e.lotteryId, e.data).done(function(t) {
              //  e.handlerResult(t)
              //})
            },nowQuickText,nowQuickBall,nowRate);
          })
        })  
      },
      loadTabs:function() {
        var controlShow = function(tag) {
          var shuzishow = function() {
            $('#j-all> .combined').hide();
            if ($('#combined-form').hasClass('active')) {
              $('#j-all').show();
              $('#j-all> table').hide();
              $('#j-all> table[rel="shuzi"]').addClass('now').show();
            }else {
              $('#j-all> table').hide();
              $('#j-sd .table tr.zonghe').hide();
              $('#j-sd .table tr.shuzi').show();
              $('#j-all> table[rel="shuzi"]').addClass('now').show();
            }
          }
          var shulonghu = function() {
            $('#j-all> table').hide();
            $('#j-all> .combined').hide();
            $('#j-all> .combined[rel="longhu"]').addClass('now').show();
            $('#j-all> .combined[rel="longhu"] table').show();
          }
          var shuniuniu = function() {
            $('#j-all> .combined').hide();
            if ($('#combined-form').hasClass('active')) {
              $('#j-all').show();
              $('#j-all>  table').hide();
              $('#j-all>  table[rel="niuniu"]').addClass('now').show();
            }else {
              $('#j-all>  table').hide();
              $('#j-all>  table[rel="niuniu"]').addClass('now').show();
            }
          }
          var shuzhongyi = function() {
            $('#j-all> .combined').hide();
            if ($('#combined-form').hasClass('active')) {
              $('#j-all').show();
              $('#j-all>  table').hide();
              $('#j-all>  table[rel="zhongyi"]').addClass('now').show();
            }else {
              $('#j-all>  table').hide();
              $('#j-all>  table[rel="zhongyi"]').addClass('now').show();
            }
          }
          var shusuohua = function() {
            $('#j-all> .combined').hide();
            if ($('#combined-form').hasClass('active')) {
              $('#j-all').show();
              $('#j-all>  table').hide();
              $('#j-all>  table[rel="suoha"]').addClass('now').show();
            }else {
              $('#j-all>  table').hide();
              $('#j-all>  table[rel="suoha"]').addClass('now').show();
            }
          }
          var shuangmian = function() {
            $('#j-all> table').hide();
            $('#j-all> .combined').hide();
            $('#j-all> .combined[rel="shuangmian"]').addClass('now').show();
            $('#j-all> .combined[rel="shuangmian"] table').show();
          }
          var allback = {
            'shuzi':shuzishow,
            'longhu':shulonghu,
            ...
            'zhongyi':shuzhongyi,
            'suoha':shusuohua,
            'shuangmian':shuangmian
          }
          if (typeof allback[tag] !='undefined') {
            $('.mt10 .now').removeClass('now');
            allback[tag]();
          }else {
            $('.mt10 .hide').removeClass('now').hide();
            $('.mt10 #'+tag+'').addClass('now').show();
            if ($('.button-secondary-group button.button-current').data('mode')=='normal') {
              $('.mt10 #'+tag+' .j-odds').show();
            }else {
              $('.mt10 #'+tag+' .j-odds').hide();
            }
            return;
          }
        }

        $('.tabchange').off('click').on('click','li',function() {
          var nt = $(this).attr('rel');
          if (nt=='niuniu' && !$('#combined-form').hasClass('active')) {
            return;
          }
          $('.tabchange li.on').removeClass('on');$(this).addClass('on');
          ////console.log($(this).attr('rel'));
          controlShow($(this).attr('rel'));
        })
      },
      showMsg:function(a) {
        var e = this;
      ...
      loadltid:function() {
        ...
        //alert($('.ui-popup-show #klc_bet').size());
        console.log(e.data,'parlay');
        ...
        setTimeout(function() {
          if ($('.ui-popup-show #klc_bet').size()>0) {
            $('.ui-popup-show #klc_bet').val($('#j-lm .sync-bet').val());
          }
        },300)
			},
      directpatlay:function() {
        alert('');
      },
			...
			openQuickDialog: function(e,about,tag,rate) {
				var t = this,
					r = "d-parlay-confirm-q";
				this.$confirmDialog = n.dialog({
					id: r,
					title: '快速投注',
					content: t.getQuickHtml(about,tag,rate),
					ok: function() {
						var n = this;
						if (!t.beforeConfirm()) return !1;
						t.beforeBet(n), e && e.call(n)
			...
			bet: function(e, t) {
				//console.log('betbetbetbet');
        return i.ajax({
					url: "/yx/u/api/xjw-lottery/bet?",
					data: JSON.stringify({
						lotteryId: e,
						betParameters: t
					})
        ...
        ////console.log(this.records,'getPostData');
			...
			getQuickHtml: function(about,tag,rate) {
				var tpl = [
          '<div><div><span>',about,'【',tag,'】 @',rate,'</span></div><div>金额：<input type="text" id="quick_money" onkeyup="if(this.value.length==1){this.value=this.value.replace(\/[^1-9]\/g,\'\')}else{this.value=this.value.replace(\/\\D\/g,\'\')}" onafterpaste="if(this.value.length==1){this.value=this.value.replace(\/[^1-9]\/g,\'\')}else{this.value=this.value.replace(\/\\D\/g,\'\')}" name="" value="" /></div></div>'  
        ];
				return tpl.join('')
			},
			...
			getBetInfo: function() {
				var e = this;
				return i.ajax({
					url:  "/yx/u/api/xjw-lottery/init-game-lottery",
					// url: "./tests",
					data: JSON.stringify({
      ...
      getBonusInfo: function() {
				var e = this;
        var re = /\d+/g;
        var found = String(location.pathname).match(re);
				var name = $('.ltlogo > span').eq(0).attr('rel').split('xjw')[0].toUpperCase();
        // if (found.length>0) {
        //   return i.ajax({
        //     url:  "/yx/u/api/game-lottery/openIssues",
        //     type: "GET",
        //     // url: "./tests",
        //     data: {
        //       id: found[0],
        ...
        // }
				return $.ajax({
					url:  "/yx/u/api/game-lottery/static-open-code",
					type: "post",
					dataType: 'json',
					// url: "./tests",
					data: {
      ...
      getMaData: function() {
        //console.log(this.getNowLotteryId(),'getNowLotteryId');
        return $.ajax({
					url:  "/yx/u/api/game-lottery/search-zhCm?id="+this.getNowLotteryId(),
					type: "get",
					dataType: 'json',
					// url: "./tests",
					data: {
      ...
      nowMaMap:{},allRateMap:{},
      getMaMap: function() {
        var alljmap = $('#j-all .sm-table .j-betting tbody tr:not(.bo-head)');
        if (alljmap.length==0) {
          alljmap = $('#parlay-ctn .j-betting tbody tr:not(.bo-head)');  
        }
        var allrdict ={},alljrate={};
        var t = this;
        $.map( alljmap, function( el ) {
          var chtag = String($(el).parent().parent().find('thead tr').text()).replace('中三球','中三').replace('前三球','前三').replace('后三球','后三').replace(/\n/g,'').replace(/\s/g,'');
          var sectag = String($(el).find('td').first().text()).replace('总和','').replace(/\n/g,'').replace(/\s/g,'');
          var chobj = {};
          var endtag = chtag+'_'+sectag;
          endtag = endtag.replace('和_和','和_').replace('冠亚_冠亚','冠亚和_');
          chobj[''+chtag+'']= $(el).data('id');
          alljrate[''+$(el).data('id')] = $(el).data('odds');
          allrdict[''+endtag] = $(el).data('id');
          return true;
        });

        this.nowMaMap = allrdict;
        this.allRateMap = alljrate;
        //console.log(allrdict,'allrdictallrdictallrdict');
      },
      getOrderRecent: function() {
				var e = this;
        var re = /\d+/g;
        var found = String(location.pathname).match(re);
        ////console.log('getOrderRecent',$('.ltlogo > span').attr('rel'));
        if (found.length>0) {
          return i.ajax({
            url:  "/yx/u/api/game-lottery/search-order",
            type: "GET",
            // url: "./tests",
            data: {
              lottery: String($('.ltlogo > span').attr('rel')).toUpperCase(),
              sTime: moment().format('YYYY-MM-DD')+ " 00:00:00",
              eTime: moment().add(1, 'days').format('YYYY-MM-DD') + " 23:59:59",
              page:0,
              size:10
            },
            success: function(oren) {
      ...
      getNowLotteryId:function() {
        ...
        if (found.length>0) {
          ...
          return -1
            ...
            //console.log(f,'handlerResulthandlerResult');
					switch (f) {
						case 1:
							l = a.bet.success, this.afterBet(l);
							$('.now .table-current').removeClass('table-current');
              break;
						case 4:
						case 5:
						case 6:
					...
					typeof f !== 'undefined' && f.isSoundOn && u.play(p),typeof resethighdot !== 'undefined' ? resethighdot() : !0,
					n.msg(l, 2500)
			...
			updateAmount: function(t) {
				function i(t, n) {
          ////console.log(r + n,n);
					e("#j-balance").text(r + t), e("#j-orders").text(r + n)//,e('[data-field="lotteryBalance"]').text(r + t)
				}

				var n = this,
					r = this.cfg.currency;
				t ? (t = t.Obj, i(t.Balance, t.NotCountSum)) : e.post("/yx/u/api/xjw-lottery/GetBalance", {
