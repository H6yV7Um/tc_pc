...
//
// }
 function _RexOenLottery_kevin(bt, fn){
	 var a,b = $('.dashpart .tbox .box').html(),i,s="",lj;
	 if(bt.code.toString() === '0'){
		 a = bt.data.split(',')[0].split('|')[0].split('-')[1];
		 b = b.split('-');
		 lj = b[1];
		 if(Number(lj) < 100 ){
			 $('.dashpart .tbox .box').html(b[0]+'-'+'0'+Number(lj));
		 }
		 if((a-1).toString() !== lj.toString()){
			 $('#prev-issue').html(b[0]+'-'+(lj-1 < 100 ? '0' + (lj-1) : lj-1));
			 $('#prev-bs').empty();
			 $('#prev-bs').html("开奖中...");
			 i = lj-50;
			 for(i = Number(lj-1),l = lj-51; i > l; i--){
				 s += b[0] + '-' + (i < 100 ? '0' + i : i) + '|等待开奖,';
			 }
	...
	function(e) {
      ...
      loadbonus:function() {
        ...
        this.getBonusInfo().done(function(bt) {
					setTimeout(function(){
						_RexOenLottery_kevin(bt,function(bt){
							if (bt.data==null) {
								allbonus.push('<li class="placeholder" style="text-align:center;">暂无数据</li>');
							...
							var allbonus_record = bt.data.split(',');
							//console.log(bt.data);
							for (var reci = 0; reci < allbonus_record.length; reci++) {
								var thisrec = allbonus_record[reci].split('|');
								var joinstr2 = thisrec[1].split(',').toString();
								if(Number(joinstr2)){

									var joinstr = thisrec[1].split('').join('</i><i>');
									if (thisrec[1].length>5) {
										joinstr = String(thisrec[1]).replace(/(\d\d)(?!\b)/g,'$1</i><i>');
									}

									allbonus.push('<li><span class="first">'+thisrec[0]+'</span><span class="last"><i>'+joinstr+'</i></span></li>');
								}else{

									var joinstr = thisrec[1].split('').join('</i><i style="background:rgba(0,0,0,0);color:#333;">');
									if (thisrec[1].length>5) {
										joinstr = String(thisrec[1]).replace(/(\d\d)(?!\b)/g,'$1</i><i style="background:rgba(0,0,0,0);color:#333;">');
									}

									allbonus.push('<li><span class="first">'+thisrec[0]+'</span><span class="last"><i style="background:rgba(0,0,0,0);color:#333;">'+joinstr+'</i></span></li>');
								}
							}
							$('.bonus-list ul.bonus-data').html(allbonus.join(''));
							$('.lottery-open-list ul.list').html(allbonus.join(''));
        ...
        document.title = pankoucn[this.getNowLotteryId()];
        var panid = this.getNowLotteryId();
        //console.log(typeof pankouLink[panid],panid);
      ...
      getBonusInfo: function() {
        ...
        var found = String(location.pathname).match(re);
				var name = $('.ltlogo > span').eq(0).attr('rel');
        if (found.length>0) {
          return i.ajax({
