define(["jquery", "/public/tjs/lottery.js"], function(e, t) {
	...
	return n = t.extend({
        ...
        ////console.log('aaaaaaaaaaa',i.lotteryId);
				r.setLiveUrl(i.lotteryId), r.ballTpl = e("#tpl-prev-balls").html(), r.$selectedCount = e("#j-selected-count"),$('.ltlogo > span').attr('rel',String(i.lotteryId).toLowerCase()).addClass(String(i.lotteryId).toLowerCase().replace('3d','d')), r._events();
        ////console.log($('.ltlogo > span').attr('rel'),'rrrrrrrrrrrrrrr');
			})
		},
		close: function() {
      ...
      var t = this,
            ...
            if(this.options.lotteryId == "pc_28"){
                ...
                ////console.log('aaaaaaaaaaaaaaaaaaaa',this.isQuickMode,this.$ctn.find(".j-betting .fb"),".j-betting tr." + i+",.j-betting li." + i);
                r = this.isQuickMode ? this.$ctn.find(".j-betting tr." + i+",.j-betting li." + i) : this.$ctn.find(".j-betting .fb");
                ////console.log(r.size());
                if ($('.mt10').size()>0) {
                  r = this.isQuickMode ? $(".mt10 .now .j-betting tr." + i+",.mt10 .now .j-betting li." + i): $('.mt10 .now .j-betting .fb');
                }
                this.records = r.map(function(n, r)
                {////处理点击
                    var i = e(this),
                        s, o, u, a;
                    t.isQuickMode ? a = t.amount : (i = e(this).closest("tr"), a = parseInt(e.trim(i.find("input").val()), 10));
		...
		afterRefresh: function(e) {
			...
			var t = this;
			var ee = e;
      //console.log('开奖了','loadbonus');
      //console.log(e,'afterRefresh');
      t.loadbonus(t, e);
      
      //alert('11');
			this._updateRanking(e.ChangLong), this._updateSummary(e), this._updateLuZhu(), this._updateHitBalls(e.ZongchuYilou), this._renderBall(e), this.isOpen && this._updateLines(e.Lines), this.refreshTimer.update = function(e) {
				if(e % 10 === 0 && !ee['_ke_isopenlottery']){
					//console.log(ee,'afterRefresh');
          t.loadbonus(t, ee);
				}
      ...
      $('.footer-bar-deal #mobile_dealcount', window.parent.document).html(e);
      if (typeof highdot !== 'undefined') {
        highdot();
      }
		},
      ...
      /////加载调用  捕获点击对象
			var t = this,
				n = e("#play-tab");
      ////console.log('initBar',typeof initBar);
      initBar();
			n.on("click", "li", function(n) {
				n.preventDefault();
        var nowtarget = $(n.target).data('target');
        if (typeof nowtarget =='undefined') {
          nowtarget = $(n.target).parent().data('target');
        }
        ////console.log(nowtarget,'nowtargetnowtarget');
        var nowtag = $('.tabchange li.on').attr('rel');
        if (nowtag!='shuzi' && nowtag!='longhu') {
          if (nowtarget == '#j-sd') {
            t.showMsg('此玩法暂无双面盘');
            return;
          }
          ...
          if (nowtag=='longhu') {
            $('#j-sd .table tr.shuzi').hide();
            $('#j-sd .table tr.zonghe').show();
          }
          if (nowtag=='shuzi') {
        ...
        $('.j-betting li.table-current').removeClass('table-current');
        $('.choosetag li.highlight').removeClass('highlight');
        $('.footer-bar-deal #mobile_dealcount', window.parent.document).html(0);
        $('ol.j-betting').attr('count',0);
        if (typeof $(n.target).parent().data('target') !='undefined') {
          var nowdivid = $(n.target).parent().data('target');
        }

        ////console.log('change',nowtag);


        //$('.choosetag li').first().addClass('active');
        $('topfive li.'+$('.choosetag li').first().attr('rel')).addClass('active');
				var r = e(this),
					i = e(r.data("target"));
				...
				if(t.isRengXuan){
					$(".button-secondary-group button[data-mode='normal']").trigger("click");
					t._RengXuan();
				}
				if(t.isZhiXuan){
					$(".button-secondary-group button[data-mode='normal']").trigger("click");
        ...
        if (parseInt(r.data("np"),10)==0) {
          $('#luzhu').addClass('onlyluzhu');
        }else {
          $('#luzhu').removeClass('onlyluzhu');
        }
        ////console.log('loadbonusloadbonus');
				// if (typeof t.loadbonus !='undefined') {
        //   t.loadbonus();
        // }
        t.loadTabs();
        ////console.log(i,'loadTabsloadTabsloadTabs');
        if (i.length) {
					t.$ctn = i;
					var s = t.isQuickMode ? 1 : 0;
                    ///mark
					////console.log(t.isQuickMode,s,'isQuickModeisQuickMode');
      ...
      $('.j-betting li').on("click",function() {});
        ...
        ////console.log(s);
			...
			}),this.$doc.on("click", ".j-betting li", function(n) {
				n.preventDefault();
				var r = e(this);
				if (t.isQuickMode) {
					if(t.isOpen==true){
					//var s = r,
					 var o = e.trim(r.find(".table-odd").text());
					if (!o) return;
					 r.toggleClass(i),r.parent().attr('count',r.parent().find('.'+i).size()), t.getData()//s.toggleClass(i),
					}
				} else r.find(".input").focus()
			}), this._tabs()
		},
		_toggleElements: function() {
      ////console.log('_toggleElements_toggleElements',this.isRengXuan,this.isZhiXuan);
			var e = this.$ctn.find(".j-odds"),
				t = this.$doc.find(".quick-form"),
				n = this.$doc.find(".normal-form"),
				r = "is-quick-mode",
				i = "is-normal-mode";
      ////console.log(this.isQuickMode);
      if (this.isQuickMode) {
        $('.mt10 .now .j-odds').hide();
      }else {
        $('.mt10 .now .j-odds').show();
      }
		...
		    }
			...
			this.$play.find("[data-id]").each(function() {
				var n = e(this),
					r = "j" + n.attr("data-id"),
					i = t[r];
				i = i > 0 ? i : 0, n.attr({
      ...
      var formatTime = function(str) {
        ...
        newstr=newstr.replace(dotexp,"$1:");
        return newstr;
      }
//			if(!t.IsLogin){
...
//			}else{
			...
			n.closeTimer.update = function(t) {
	            function s(t,timer) {
	                ...
	                //}),//{{days}}{{hours}}
	                //r = n.tpl.to_html("{{minutes}}{{seconds}}", s),
                  timer.find('em.min').text(s.minutes),
                  timer.find('em.sec').text(s.seconds);
                  e("#close-timer").attr('m',t);
									////console.log(t);
                  //r1 = n.tpl.to_html("{{days}}{{hours}}{{minutes}}{{seconds}}", s),
                  //$('lottery #topmcounter', window.parent.document).html(formatTime(r)),$('.footer-bar-deal #mcounter_other', window.parent.document).html(formatTime(r));
	                //timer.text(r)
	            }
				s(t,e("#close-timer"));
        ...
        ////console.log(t.OpenCount,'OpenCountOpenCountOpenCount');
      	e("#close-timer").html('<span class="days"><em class="dots">:</em></span><em class="min">00</em><em class="dots">:</em><em class="sec">00</em></b>');
        ...
        function s(t,timer) {
            ...
            if (parseInt(e("#close-timer").attr('m'),10)==0) {
              e("#award-timer").attr('s',t);
              if (typeof e("#award-timer").attr('s') != 'undefined' && Number(e("#award-timer").attr('m'))-Number(e("#award-timer").attr('s'))==5) {
                n.open();
              }
            }else {
              e("#award-timer").attr('m',t);
            }
						s = n.utils.secondsFormat(t);
            timer.find('em.min').text(s.minutes);
            timer.find('em.sec').text(s.seconds);
...
//					}
		...
		_genderedRengXuanBalls: function(t) {
            ...
            if (typeof tht == 'undefined') {
              tht = $(this).closest("ol").attr("data-tabletype");
              tot = $(this).closest("ol").attr("data-total");
            }
            if (tit=='') {
              tit = $(this).closest("ol").prev().text();
            }
            if (ods=='') {
              ods = $(this).find(".odds-text").text();
            }
            ////console.log(tht,tot,tit,ods);
		...
		_genderedZhiXuanBalls: function(t) {
            ...
            //console.log(bn,tit,ods);
            /*if (typeof tht == 'undefined') {
              tht = $(this).closest("ol").attr("data-tabletype");
              tot = $(this).closest("ol").attr("data-total");
            }*/
            if (tit=='') {
              tit = $(this).closest("ol").prev().find('em').text();
            }
            if (ods=='') {
              ods = $(this).find(".odds-text").text();
            }
		...
		_RengXuan: function() {
			...
			var n = this._RengXuanConfig,r = "j-current-odds";
      t.$doc.off("click", ".j-highlights-tb li").on({
				click: function(t) {
					var ths = e(this);
					var thf = ths.closest("ol");
					var total = thf.attr("data-total");
					var tr_list = thf.find('li');
