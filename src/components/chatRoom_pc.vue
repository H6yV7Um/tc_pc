<template>
  <div class="chatRoomBox" :class="{'newMessage':newMessage}">
        <span  v-if="!showRoom" class="door" @click="showRoom=true;IsAddRoom=true"><img src="/static/img/chatRoom_show.png" alt=""></span>
    <div v-if="IsAddRoom" class="chatRoom" :class="{'closeRoom':!showRoom}">
      <!--<span class="close" @click="showRoom=false">X</span>-->
      <div class="title">消息推送<span class="btn_title" ><span @click="data=[];clearNotice()">清空</span><span @click="showRoom=false;newMessage=false">关闭</span></span></div>
        ...
        <ul>
          ...
          <li v-for="(v,i) in data" :key="i" :class="{'right':v.type===1,'systemMessage':v.type===3}">
            <div class="box">
              ...
              <!--系统消息-->
              <div v-else class="systemItem" :class="{'withdraw':v.messageType===1,'recharge':v.messageType==0}">
                <div class="systemTitle" @click="openPage('/userMessage#page=0_message')">系统消息: {{v.title}}<br>{{v.time}}</div>
          ...
          <li v-if="showNotice">
            ...
            <ul class="noticeMain colorY bgcolorA" >
              <li>
                <a href="/userMessage#page=0_message" target="_blank" >系统消息 {{noticeList1[0] && noticeList1[0].time}}</a>
...
<script>
  ...
  *
   */
	...
	//造数据 type:1本人发的 2别人发的 3中奖信息
	// var obj = [
	// 	{type: 1, time: 1, name: 'Alan', message: 'helloworld'},
	// 	{type: 2, time: 5, name: 'Alan', message: 'helloworld1'},
	// 	{type: 2, time: 4, name: 'Alan', message: 'helloworld2'},
	// 	{type: 1, time: 3, name: 'Alan', message: 'helloworld3'},
	// 	{type: 2, time: 2, name: 'Alan', message: 'helloworld4'},
	// 	{type: 1, time: 6, name: 'Alan', message: '哟，好巧哦，你也在装逼哟，钓鱼岛是中国的'},
	// ]
	//读cookie
      ...
      *  就会在一瞬间里面被认为是抬起了，然后又按下了
       */
			...
			socketInit() {
				...
				// };
				let that = this;
				console.log(location.origin.replace('http', 'ws') + '/message/chat?sessionId=' + getCookie("sso_session_uid"))
      ...
      // 没有登录的时候，不初始化socket
		...
		mounted() {
			if (!(String(location.href).includes('register_mobile'))) {
				init.init(this);
				init.getNotice(this);
			}
		},
