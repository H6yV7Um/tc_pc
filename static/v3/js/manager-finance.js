...
var initThisPage01_fin_new = function () {
  var onlyidDictCheck = {}, tempAllChannel = [], nowpayment;
  var isRandom = false; //选择二级充值方法 true为随机自动选择，false为人工选择，默认第一个
  var bigChannel = {
    'wechat': [], 'alipay': [], 'qqpay': [], 'unionpay': [], 'yunscan': [], 'onlinepay': []
  }

  var thisContent = $('[data-init="content"]').last();
  //辅助类
  var base = {
    doMove(e, c, d) {
      var g = 0;
  ...
  //初始化帮助
  var initHelpSlide = function () {
    $('.deposit_slider .sildeTabr').off('click').click(function () {
      if (!base.isnext) {
        return false;
      }
      base.isnext = false;
      ...
      $('.deposit_slider .sildeTabl').css('opacity', "1");
      var defnum = 4;
      if ($('.deposit_slider .sildeTabMain > ul.nowtab').hasClass('wxtab')) {
        defnum = 5;
      }
      if (base.tabtype === 0) {
        if (base.idx >= defnum) {
        ...
        base.move($('.deposit_slider .sildeTabMain > ul.nowtab').eq(0), {"left": -base.idx * 230}, function () {
          base.isnext = true;
        });
      } else {
        if (base.idx >= defnum) {
    ...
    $('.deposit_slider .sildeTabl').off('click').click(function () {
      if (!base.isnext) {
        return false;
      }
      base.isnext = false;
      base.idx--;
      $('.deposit_slider .sildeTabr').css('opacity', "1");
      if (base.idx <= 0) {
        base.idx = 0;
        $('.deposit_slider .sildeTabl').css('opacity', "0.4");
      }
      if (base.tabtype === 0) {
        base.move($('.deposit_slider .sildeTabMain > ul.nowtab').eq(0), {"left": -base.idx * 230}, function () {
          base.isnext = true;
  ...
  var getChannels = function () {
    PaymentCtrl.requestAllMethod({
      beforeSend: function () {
        ...
        //console.log('go read methods');
      },
      success: function (response) {
        buildChannels(response);
      },
      complete: function () {
        App.unblockUI(thisContent);
      }
  ...
  var hasKeyword = function (a, b) {
    if (String(a).indexOf(b) > -1) {
      return true;
    }
    return false;
  }
  var orgByName = function (a, b, c) {
    var sortname = '';
    (typeof a == 'undefined' && typeof b != 'undefined' && typeof c == 'undefined') ? sortname = 'thirdpay' : (typeof a != 'undefined' ? sortname = 'qrcodepay' : sortname = 'transpay')
    return sortname
  }
  var sortByName = function (a, b, c) {
    var sortname = '';
    //(typeof a =='undefined' && typeof b !='undefined' && typeof c =='undefined') ? sortname= 'thirdpay' : (typeof a !='undefined' ? sortname= 'qrcodepay' : sortname = 'transpay')
    (typeof a != 'undefined' && hasKeyword(a, '微信')) ? sortname = 'wechat' : !0;
    (typeof b != 'undefined' && hasKeyword(b, '微信')) ? sortname = 'wechat' : !0;
    (typeof c != 'undefined' && hasKeyword(c, '微信')) ? sortname = 'wechat' : !0;

    (typeof a != 'undefined' && hasKeyword(a, 'QQ')) ? sortname = 'qqpay' : !0;
    (typeof b != 'undefined' && hasKeyword(b, 'QQ')) ? sortname = 'qqpay' : !0;
    (typeof c != 'undefined' && hasKeyword(c, 'QQ')) ? sortname = 'qqpay' : !0;

    (typeof a != 'undefined' && hasKeyword(a, '支付宝')) ? sortname = 'alipay' : !0;
    (typeof b != 'undefined' && hasKeyword(b, '支付宝')) ? sortname = 'alipay' : !0;
    (typeof c != 'undefined' && hasKeyword(c, '支付宝')) ? sortname = 'alipay' : !0;

    (typeof a != 'undefined' && hasKeyword(a, '银联')) ? sortname = 'unionpay' : !0;
    (typeof b != 'undefined' && hasKeyword(b, '银联')) ? sortname = 'unionpay' : !0;
    (typeof c != 'undefined' && hasKeyword(c, '银联')) ? sortname = 'unionpay' : !0;

    (typeof a != 'undefined' && hasKeyword(a, '云闪付')) ? sortname = 'yunscan' : !0;
    (typeof b != 'undefined' && hasKeyword(b, '云闪付')) ? sortname = 'yunscan' : !0;
    (typeof c != 'undefined' && hasKeyword(c, '云闪付')) ? sortname = 'yunscan' : !0;

    (typeof a != 'undefined' && hasKeyword(a, '网银')) ? sortname = 'onlinepay' : !0;
    (typeof b != 'undefined' && hasKeyword(b, '网银')) ? sortname = 'onlinepay' : !0;
    (typeof c != 'undefined' && hasKeyword(c, '网银')) ? sortname = 'onlinepay' : !0;

    return sortname
  }
  var buildChannels = function (res) {
    tempAllChannel = res.data.qrCodeList;
    tempAllChannel = tempAllChannel.concat(res.data.thridList);//.concat(res.data.transferList);
    //console.log(res.data.transferList,'res.data.transferList');
    var maxTransferList = [], nowtrans = res.data.transferList;
    for (var k = 0; k < nowtrans.length; k++) {
      var inserrandom = _.random(0, 2)
      var wechat_tran = jQuery.extend({}, nowtrans[k]);
      var alipay_tran = jQuery.extend({}, nowtrans[k]);
      var unionpay_tran = jQuery.extend({}, nowtrans[k]);
      var fixname = nowtrans[k].cardName;
      fixname = String(nowtrans[k].cardName).split('')[0]+'**';
      wechat_tran.cardName = '微信_' + fixname;
      wechat_tran.codeType = 0;
      wechat_tran.pid = nowtrans[k].id;
      wechat_tran.offlineType = 'weixin';
      alipay_tran.cardName = '支付宝_' + fixname;
      alipay_tran.codeType = 1;
      alipay_tran.pid = nowtrans[k].id;
      unionpay_tran.cardName = '银联转账_' + fixname;
      unionpay_tran.codeType = 5;
      unionpay_tran.pid = nowtrans[k].id;

      //if (inserrandom==0) {
      maxTransferList.push(wechat_tran)
      //}
      //if (inserrandom==1) {
      maxTransferList.push(alipay_tran)
      //}
      //if (inserrandom==2) {
      maxTransferList.push(unionpay_tran);
      //}
    }
    tempAllChannel = tempAllChannel.concat(maxTransferList);
    //tempAllChannel.concat(res.data.transferList);
    //console.log(tempAllChannel,tempAllChannel.length);
    for (i = 0; i < tempAllChannel.length; i++) {
      var nowloop = tempAllChannel[i];
      var nowtypestr = sortByName(nowloop.nickName, nowloop.name, nowloop.cardName);
      var noworgtype = orgByName(nowloop.nickName, nowloop.name, nowloop.cardName);
      var nowrandom_index = 0;
      //console.log(nowtypestr);
      //console.log(nowloop.nickName,nowloop.name,nowloop.cardName);
      nowloop['mytype'] = nowtypestr;
      nowloop['otype'] = noworgtype;
      if (nowtypestr != '') {
        bigChannel[nowtypestr].push(nowloop);
        if (bigChannel[nowtypestr].length > 1) {
          nowrandom_index = _.random(0, bigChannel[nowtypestr].length - 1)
        }
      }
      //console.log(nowrandom_index);
      if (typeof bigChannel[nowtypestr] != 'undefined') {
        $('.deposit_item .typ_list li[rel="' + nowtypestr + '"]').attr('now', nowrandom_index).addClass('nowactive');
      }

    }
    if ($('.typ_list li:not(.nowactive)').size() > 0) {
      $('.typ_list li:not(.nowactive)').hide();
    }
    console.log(bigChannel, 'bigChannelbigChannel');
    chooseChannel('wechat');
  }
  var resetToFirst = function () {
    $('#amount_money').val(100);
    demoStep(0);
    $('.deposit_second').hide();
    $('#amount_fuyin').val('');
    $('.deposit_second .confirm_bank').attr('class', 'confirm_bank b2');
    $('.deposit_next').show();
    $('.deposit_last').hide();
    $('#deposit_newtext').val('');
    $('#deposit_newform').attr('action', '');
    $('#deposit_help_slider').removeClass('deposit_slider_alipay').removeClass('deposit_slider_wechat').hide();
  }
  var bindChannelEvt = function () {
    $('.deposit_item .typ_list').off('click').on('click', 'li', function () {
      if (!$(this).hasClass('nowactive')) {
        App.alert('info', '消息提示', '充值渠道不可用！');
        return false;
      }

      resetToFirst();
      chooseChannel($(this).attr('rel'));
    });

    $('.deposit_item #main_fire').off('click').on('click', function () {
      var nowmnum = parseInt($('#amount_money').val(), 10);
      if (nowmnum < parseInt($('#amount_money').attr('lmin'), 10) || nowmnum > parseInt($('#amount_money').attr('lmax'), 10)) {
        App.alert('info', '消息提示', '充值金额需要大于等于' + $('#amount_money').attr('lmin') + '或者小于等于' + $('#amount_money').attr('lmax') + '！');
        return false;
      }
      if ($('#amount_fuyin').val() == '' && $('.deposit_fuyin:visible').size() > 0) {
        App.alert('info', '消息提示', '充值附言不能为空！');
        return false;
      }
      //'/payment/request-transfer-pay'
      var urlpath = '/payment/request-thrid-pay';
      var nowtype = 'thirdpay';
      //api/payment/request-transfer-pay
      var pdata = {
        pid: nowpayment['code'],
        bankco: $('.deposit_banks li.on').attr('rel'),
        amount: $('#amount_money').val()
      }
      if (nowpayment['otype'] == 'qrcodepay') {
        urlpath = '/payment/request-transfer-pay';
        nowtype = 'qrcodepay';
        pdata = {
          pid: '',
          amount: $('#amount_money').val(),
          fuYan: $('#login_header .say_hello .nickname').attr('title') + ':',
          payWay: nowpayment['codeType'],
          qrid: nowpayment['id']
        }
        $('#main_finished').css('display', 'inline-block');
        $('#main_finish').hide();
        //pid:
        //amount:200
        //fuYan:alphago:
        //payWay:4
        //qrid:0e854c9f7b424e7099aabdb16b0b1d45
      }
      if (nowpayment['otype'] == 'thirdpay') {
        $('#main_finish').css('display', 'inline-block');
        $('#main_finished').hide();
      }
      if (nowpayment['otype'] == 'transpay') {
        urlpath = '/payment/request-transfer-pay';
        nowtype = 'transpay';
        pdata = {
          pid: nowpayment['pid'],
          amount: $('#amount_money').val(),
          fuYan: $('#login_header .say_hello .nickname').attr('title') + ':' + $('#amount_fuyin').val(),
          payWay: nowpayment['codeType']
        }
        if (typeof nowpayment['offlineType'] != 'undefined') {
          pdata['offlineType'] = nowpayment['offlineType'];
        }
        $('#main_finished').css('display', 'inline-block');
        $('#main_finish').hide();
        //pid:
      ...
      console.log(nowtype,'nowtypenowtypenowtype');
      Will.ajax(pdata, Route.PATH + urlpath, function (data) {
        $('#deposit_newtext').val(data.text);
        demoStep(1);
        //confirm_number
        $('.deposit_second[rel="' + nowtype + '"]').show();
        $('.deposit_second[rel="' + nowtype + '"]').find('.confirm_bank').attr('class', 'confirm_bank ' + $('.deposit_banks li.on').attr('class'))
        $('.deposit_second[rel="' + nowtype + '"]').find('.confirm_number').html(Number(data.amount).toFixed(2));
        $('.deposit_second[rel="' + nowtype + '"]').find('.confirm_number').next().attr('data-clipboard-text', Number(data.amount).toFixed(2));
        $('.deposit_second[rel="' + nowtype + '"]').find('.confirm_czno').html(data.billno);
        $('.deposit_second[rel="' + nowtype + '"]').find('.confirm_czno').next().attr('data-clipboard-text', data.billno);
        if (data.bankId == null) {
          $('.deposit_second[rel="' + nowtype + '"]').find('.choosebank_items').hide();
        } else {
          $('.deposit_second[rel="' + nowtype + '"]').find('.choosebank_items').show();
        }

        if (nowtype == 'qrcodepay') {
          $('.deposit_second[rel="' + nowtype + '"]').find('.qrcode_confirm img').attr('src', 'data:image/gif;base64,' + nowpayment['fileByte']);
          $('.deposit_second[rel="' + nowtype + '"]').find('.choosebank_items').hide();
          $('.deposit_item #main_finish').hide();
          $('.deposit_item #main_finished').removeClass('notready');

          $('.deposit_item #main_finished').attr('href', '/userAccount#page=3_finance');
          //choosebank_items
        }
        if (nowtype == 'transpay') {
          //console.log(nowpayment.cardName);
          $('.deposit_second[rel="' + nowtype + '"]').find('.confirm_czname').html(String(nowpayment.cardName).split('_')[1]);
          $('.deposit_second[rel="' + nowtype + '"]').find('.confirm_czname').next().attr('data-clipboard-text', String(nowpayment.cardName).split('_')[1]);
          $('.deposit_second[rel="' + nowtype + '"]').find('.confirm_czcard').html(nowpayment.cardId);
          $('.deposit_second[rel="' + nowtype + '"]').find('.confirm_czcard').next().attr('data-clipboard-text', nowpayment.cardId);
          $('.deposit_second[rel="' + nowtype + '"]').find('.confirm_bank').attr('class', 'confirm_bank b' + nowpayment.bankId)
          $('.deposit_item #main_finish').hide();//$('#amount_fuyin').val('');
          $('.deposit_item #main_finished').removeClass('notready');
        ...
        if (nowtype == 'thirdpay') {
          $('.deposit_item #main_finished').removeAttr('href');
        }
        $('.deposit_next').hide();
        $('.deposit_last').show();
        ...
        if (nowtype == 'transpay') {
          setTimeout(function() {
            $('#deposit_help_slider').addClass('deposit_slider_alipay').show();  
          },300);
        }
        $('#deposit_newform').attr('action', data.link);
      });
    });

    $('.am_quick a').off('click').on('click', function () {
      if ($('#amount_money').val() == '') {
        $('#amount_money').val(0);
      }
      $('#amount_money').val(parseInt($('#amount_money').val(), 10) + parseInt($(this).attr('rel'), 10));
    });

    $('.deposit_item #main_finish').off('click').on('click', function () {
      $('#deposit_newform').submit();
      $('#main_finish').hide();
      demoStep(2);
      setTimeout(function () {
        //resetToFirst();
    ...
    $('.deposit_item #main_finished').off('click').on('click', function () {
      demoStep(2);
      setTimeout(function () {
        resetToFirst();
      }, 2000);
    ...
    $('.deposit_item #main_reset').off('click').on('click', function () {
      resetToFirst();
    });
  }
  var buildBanks = function (banks) {
    var banksout = [];
    for (var i = 0; i < banks.length; i++) {
      banksout.push('<li class="b' + banks[i].bankId + '" rel="' + banks[i].code + '"><div class=""></div><em></em></li>')
    }
    $('.deposit_banks').html(banksout.join(''));
    $('.deposit_banks li').first().addClass('on');

    $('.deposit_banks').off('click').on('click', 'li', function () {
      $('.deposit_banks li.on').removeClass('on');
      $(this).addClass('on');
    });
  }

  var buildSeconds = function(seconds) {
    //console.log(seconds);
    var allsec = [];
    var getName = function(item) {
      ...
      if (typeof item.cardName !='undefined') {
        return item.cardName;   
      }
      return item.nickName;  
    }
    for (var i = 0; i < seconds.length; i++) {
      allsec.push('<li class="'+(i==0 ? 'on' :'')+'" rel="'+i+'">'+getName(seconds[i])+'</li>')
    }
    $('.sectyp_list').html(allsec.join(''));

    $('.sectyp_list').off('click').on('click','li',function() {
      var nowsecindex = parseInt($(this).attr('rel'),10);
      console.log($('.typ_list .type_on').attr('rel'),nowsecindex);
      $('.sectyp_list .on').removeClass('on');
      $('.sectyp_list li[rel="'+nowsecindex+'"]').addClass('on');
      chooseChannel($('.typ_list .type_on').attr('rel'),nowsecindex)  
    })
  }
  var chooseChannel = function (rel) {
    var nowchannel = $('.deposit_item .typ_list li[rel="' + rel + '"]');
    $('.deposit_item .typ_list li').removeClass('type_on');
    nowchannel.addClass('type_on');
    var nowindex = parseInt(nowchannel.attr('now'), 10);
    if (!isRandom) {
      nowindex = 0;  
    }
    if (arguments.length>1) {
      nowindex = parseInt(arguments[1],10);
      //console.log(nowindex,'directinedx');
    }
    //console.log('chooseChannelchooseChannel');
    //console.log(bigChannel[rel],'bigChannel[rel]bigChannel[rel]');
    nowpayment = bigChannel[rel][nowindex];
    if (arguments.length==1){
      if (bigChannel[rel].length>0 && !isRandom) {
        buildSeconds(bigChannel[rel]);     
      }else {
        $('.sectyp_list').html('');
      }
    }

    //console.log(nowindex,nowpayment, nowpayment['otype']);
    if (typeof nowpayment['banklist'] != 'undefined') {
      buildBanks(nowpayment['banklist']);
    }
    $('#amount_money').attr('lmin', nowpayment['minUnitRecharge']).attr('lmax', nowpayment['maxUnitRecharge']);
    $('.am_limits em').first().html(nowpayment['minUnitRecharge']);
    $('.am_limits em').last().html(nowpayment['maxUnitRecharge']);
    //console.log(String(nowpayment['cardName']).indexOf('_')>-1,(nowpayment['codeType']==5 || nowpayment['codeType']==0));
    if (String(nowpayment['cardName']).indexOf('_') > -1 && (nowpayment['codeType'] == '5' || nowpayment['codeType'] == '1')) {
      if (nowpayment['codeType'] == '5') {
        $('#amount_fuyin').attr('placeholder', '请输入持卡人姓名');
      }
      if (nowpayment['codeType'] == '1') {
        $('#amount_fuyin').attr('placeholder', '请输入支付宝姓名');
      }
      $('.deposit_fuyin').show();
    } else {
      $('.deposit_fuyin').hide();
    ...
    if (typeof nowpayment.banklist != 'undefined' && nowpayment.banklist.length > 0) {
      $('.deposit_item[rel="bank"]').show();
    } else {
      $('.deposit_item[rel="bank"]').hide();
    }

  }
  var demoStep = function (index) {
    $('.step_items li.nums').removeClass('on');
    $('.step_items li.nums').eq(index).addClass('on');
    //step_items
  }
  getChannels();
  bindChannelEvt();
...
var initThisPage02_fin = function () {
  ...
  thisContent01.find('input[name="submit"]').unbind().click(function () {
    ...
    if (testApply(amount, withdrawPwd)) {
      var data = {amount: Number(amount).toFixed(2), cardId: cid, withdrawPassword: md5(withdrawPwd)};
...
var initFinance = function () {
  var tabs = [initThisPage01_fin, initThisPage02_fin, initThisPage03_fin_simple, initThisPage04_fin, initThisPage05_fin, initThisPage06_fin, initThisPage01_fin_new]; //initThisPage03
