...
var UserSysMessage = function() {
    ...
    if(els().is(':hidden')) {
      ...
      //console.log(height,'bottom_heightheightheight');
  ...
  var addonlyone = function(data) {
        var count = 0;
        if (data && data.length > 0) {
            $.each(data, function(i, val) {
                if ((lastTime && lastTime >= val.time) || i>0) {
                    return;
                }
                var item =
                    '<div class="item" id="not'+val.id+'"><a href="/userMessage#page=0_message" target="_blank">\
					<div class="title">\
						<span class="type">' + DataFormat.formatUserSysMessageType(val.type) + '</span>\
						<span class="time">' + moment(val.time).format('HH:mm:ss') + '</span>\
					</a></div>\
                ...
                els().find('.list').html('').prepend(item);
                idArray.push(val.id);
                count++;
            });
            if (count > 0) {
...
var mysetCookie = function(name, value, days) {
  var d = new Date;
...
var getLeftSec = function(a) {
  ...
  if (typeof $('.lottery-navs ul').data('loading') == 'undefined') {
    ...
    typeof Api !='undefined' && Api.getCommon('getFavGame',{t:new Date().getTime()},function(res) {
      var ltall = ['<li class="title"><img src="/static/v3/images/lotteryType.png" alt="">常玩彩种</li>'],allres = res.data;
      var allopenid =[];
      for (i = 0; i < allres.length; i++) {
        if (String(allres[i].lotteryName).indexOf('微信')==-1 && allres[i].lotteryId!='113') {
          ...
          var hrefpath = '/static/lt.html#'+allres[i].lotteryId+'';
          if (location.hash!='') {
            hrefpath = '#'+allres[i].lotteryId+'';
          }
          ////console.log(hrefpath,'hrefpathhrefpathhrefpath');
  ...
  ////console.log(typeof getCookie,typeof getCookie !='undefined' && typeof getCookie('WIN-VOICE') !='undefined' && getCookie('WIN-VOICE')=='ON','oooooooooooo');
  if (typeof getCookie !='undefined' && typeof getCookie('WIN-VOICE') !='undefined' && getCookie('WIN-VOICE')=='ON') {
    $('.audio-sw').removeClass('audio-on').addClass('juston audio-off');
  }else {
    $('.audio-sw').removeClass('audio-off').removeClass('juston').addClass('audio-on');
  }
...
var sscorder = [11, 161, 151, 6, 119, 51, 711, 811, 46, 203];
...
var classicorder = [80,90,100,110,85,86,87];
...
$(document).ready(function() {
    // 线路表弹窗
    (function() {
		      var speedInit =   function(speedSrc, speedConfig) {
		          var speed = {
		              allsites: speedSrc,
		              domStyle: speedConfig,
		              speedIdx: 0,
		              init2: function() {
		                  if (this.speedIdx > this.allsites.length - 1) this.speedIdx = 0;
		                  let classStr = this.speedIdx % 2 !== 0 ?  'odd' : '';
		                  let aLink = this.speedIdx === 0 ? ('<a href="' + this.allsites[0] + '" class="detectionBtn ft16" style="width:110px; margin-right:-90px">立即进入 ></a>') : '';

		                  var html =
		                      '<div class="detectionMain_contDemo">' +
		                      '<div class="top clearfix txtl router_line ' + classStr + '">' +
		                      '<em class=" ft14">' + this.domStyle[1][this.speedIdx] + '</em>' +
		                      '<a class="detectionMain_progress bgcoloref clearfix" href="' + this.allsites[this.speedIdx] + '">' +
		                      '<div class="txtl ft14 spColor1' + this.speedIdx + '" style="width:' + this.domStyle[0][this.speedIdx] + '%">' +
		                      '</div></a></div></div>';

		                  $('.detectionMain_cont .part_cont2').append(html)
		                  if(this.speedIdx === 0) {
		                      $('.detectionMain_cont .best_route').append(html);
		                      $('.detectionMain_cont .best_route .detectionMain_contDemo').removeClass('detectionMain_contDemo');
		                      $('.detectionMain_cont .best_route .spColor10').removeClass('spColor10').addClass('spColor11');
		                      $('.detectionMain_cont .best_route .router_line').append(aLink)
		                  }
		                  this.speedIdx++;
		          ...
		          return speed;
		      }(appConfig().speedSrc, appConfig().speedConfig);
		      $('.util_prompt').hide();
		      function initImags(srcs, speedConfig) {
		          for(var i = 0; i < srcs.length; i++) {
		              var oimg = document.createElement('img');
		              oimg.src = 'http://' + srcs[i] + '/yx/' + Math.random(10) + '.html';
		              oimg.onerror = function() {
		                  speedInit.init2(srcs, speedConfig);
		              }
		              $('.speed_test .imgWrapper').append(oimg);
		          }
		      }
		//    $(".fixright-bars .router_click").click(
		      $(".fixright-bars .router_click").click(function () {
		          $('.util_prompt').show();
		          speedInit.speedIdx = 0;
		          $('.detectionMain_cont .part_cont2').html('');
		          $('.detectionMain_cont .best_route').html('');
		          $('.speed_test .imgWrapper').html('');
		          initspeeds();
		      });

		      function initspeeds() {
		          var speedSrc = appConfig().speedSrc;
		          var speedConfig = appConfig().speedConfig;
		          initImags(speedSrc, speedConfig);
		      }
		      $('.util_prompt .refresh_route').click(function() {
		          if ($('.speed_test .part_cont2 .detectionMain_contDemo').length > 5) {
		              $(".fixright-bars .router_click").click();
		              $('.util_prompt').hide();
		      ...
		      $('.speed_title .icon').click(function() {
          $('.util_prompt').hide();
      })
    })()
  ...
  var allltdict = {};
    ...
    let gameClassifyArr = [
        { name: '时时彩', idArr: [11, 161, 151, 6, 119, 161, 51, 711, 811, 46, 203,911, 205, 206, 191, 601, 61, 50] , idz: 0},
        { name: '菲律宾', idArr: [200, 201, 202] ,idz: 1},
        //    {name : '全天彩', idArr: []},
        { name: '11选5', idArr: [21, 22, 23, 24, 25, 26, 28] ,idz: 2},
        { name: '低频彩', idArr: [41, 42] ,idz: 3},
        { name: '其他', idArr: [204, 31, 32, 33, 34, 35, 36, 43, 47],idz: 4 },
        { name: '六合彩', idArr: [250] ,idz: 5},
        { name: '经典', idArr: [80, 82, 85, 86, 87, 88, 90, 100, 102 ,103, 110, 104, 105,70, 71, 72] ,idz: 6}
    ];
  ...
  Api.getCommon('onlineLotterys',{t:new Date().getTime()},function(res) {
    ...
    ////console.log(res);
    var ltall = [],fullltssc=[],trendlts=[],allres = res.data;
    var ltssc=[],ltssc1=[],ltssc2=[],ltssc3=[],ltssc4=[],ltxuan=[],ltclassic=[],ltkuai=[],ltother=[],otherdict = {};
    var hotlt = [1,9,13],newlt = [8,15,7];
    ...
    // $('#top-lt-list .sec-lt-list[rel="ssc"]').html(ltssc.join(''));
    // $('#top-lt-list .sec-lt-list[rel="guanfang"]').html(ltssc1.join(''));
    // $('#top-lt-list .sec-lt-list[rel="flb"]').html(ltssc2.join(''));
    // $('#top-lt-list .sec-lt-list[rel="xuan"]').html(ltxuan.join(''));
    // $('#top-lt-list .sec-lt-list[rel="dpc"]').html(ltssc4.join(''));
    ...
    ////console.log(allres,'allres');
      ...
      for (let z = 0; z < gameClassifyArr.length; z++) {
          ...
          let obj = tem.objArr;
          let str = '';
          if (z < gameClassifyArr.length - 1 && !!obj) {
              for (let x = 0; x < obj.length; x++) {
                  // console.log(obj[x].id,'11111111111');
                  if (obj[x].id!='250') {
                    innerWrapper.push(
                        '<li rel="' + obj[x].id + '"><a href="' + '/static/lt.html#' + obj[x].id + '">' +
                        ((obj[x].id === 911 || obj[x].id === 50) ? '<span class="icon lottery_list_newAdd">&#xe60d;</span>' : '')  +
                        obj[x].showName +
                        '</a></li>');  
                  }else {
                    innerWrapper.push(
                      '<li rel="' + obj[x].id + '"><a href="/pankou/#' + obj[x].id + '">' +
                      ((obj[x].id === 911 || obj[x].id === 50) ? '<span class="icon lottery_list_newAdd">&#xe60d;</span>' : '')  +
                      obj[x].showName +
                      '</a></li>');  
                  }
    ...
    $('#trendpage nav.alllotterys').html(trendlts.join(''));
    $('#top-lt-list').data('all',allres);
    //$('#nav-ltmenu ul').html(ltall.join(''));
    $('#nav-ltmenu').html($('#top-lt-list').html());
    getVrGame();
  ...
  $('#saveMy').on('shown.bs.modal', function() {
    ...
    //console.log($('.savemylt .seccls[rel="ssc"] ul'),'pankouuuuuuuuuuuuuuuu');
    ...
    $('.savemylt').off('click').on('click','.m_change',function() {
      //console.log($(this).attr('rel'),nowel.find('.m_on').size());
  ...
  if (location.pathname=='/yx/hbs/manager-account.html') {
    ...
    $('.newheader .inuser').show();$('.newheader .normal').hide();
  }else {
    //$('.fixedleftbar.normal').show();$('.fixedleftbar.uc').hide();
    $('.newheader .inuser').hide();$('.newheader .normal').show();
  }
  ...
  $.get("/json/online_game_user.json", function(data) {
    console.log(data);
    

    var number1 = parseInt(data.onlineUser,10);
    var number2 = parseInt(data.gamingUser,10);
    document.getElementById('counter1').innerHTML=number1;
    document.getElementById('counter2').innerHTML=number2;

    $("#counter1").numberRock({
      speed:190,
      count:parseInt(data.onlineUser,10)
    })
    $("#counter2").numberRock({
      speed:120,
      count:parseInt(data.gamingUser,10)
    })
    var t = function(name) {
      number1++;
      number2++;
  ...
  var nowchangebg = function() {
    ...
    if (typeof $.cookie !='undefined' && typeof $.cookie('DARK') !='undefined' && $.cookie('DARK') =='1') {
      $('.godark span').html('白天模式');$('.godark').addClass('isdark');
      $('.lottery_main').addClass('lottery_main_dark');
    }else {
      $('.godark span').html('黑夜模式');$('.godark').removeClass('isdark');
