...
var PopMessageDetail = function(){
    var cleanHtml = function(str) {
        ...
        text = str.replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&nbsp;/g, ' ').replace(/&amp;nbsp;/g, ' ');
        return text;
    }
		//收件箱-详细DOM
					...
					'<table class="form-control-float">'+
						'<tbody>'+
							...
							'<tr class="textarea">'+
								...
								'<td class="value">'+
									'<div id="textarea" style="font-size:14px;color:#333;height:280px;line-height:20px;overflow-y:auto" readonly="readonly" class="form-control message">' + '</div>'+
								'</td>'+
							'</tr>'+
							'<tr class="actions">'+
								...
								'<td class="value">'+
									'<div class="button-groups">'+
										formatActions +
										'<input data-id="' + data.id + '" name="delete" type="button" class="button" value="删除"/>'+
                    //'<a href="/userMessage#page=1_message" ref="/yx/hbs/manager-message.html?replay=1&nick='+formatUserValue+'#page=1" onclick="$(\'.jBox-closeButton\').click()"><input data-name="' + formatUserValue + '" name="replay" type="button" class="button" value="回复"/></a>'+
	...
	var details = function(type, data, callback) {
		Will.initBox('<i class="icon lock"></i>消息详情', initDoc(type,data,callback),750,490,function(){
            $("#textarea").html();
		    $("#textarea").append(cleanHtml(data.content));
