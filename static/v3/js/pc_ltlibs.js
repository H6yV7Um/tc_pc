    ...
    var initBox = function (data, thisContent, callback) {
      if (box == undefined) {
        ...
        box = new jBox('Modal', {
          ...
          draggable: 'title',
          content: doc,
          addClass: 'common-modal pankou-modal',
          onInit: function () {
            this.open();
  ...
  var PopOrderRecord = function () {
    ...
    var details = function (id, thisContent, callback, isAgent) {
      ...
      Will.ajax({billno: id}, url, function (data) {
        Will.initBox('订单详情', initDoc(data), 800, 540, initEvent);
      })
    }
    var doCancelOrder = function (data, thisContent, callback) {
    ...
    var chaseDetail = function (id, thisContent, callback) {
      Will.ajax({billno: id}, Route.PATH + '/game-lottery/get-chase', function (data) {
        Will.initBox('订单详情', initDoc02(data), 800, 560, initEvent02);
      });
    }
    ...
    var initEvent02 = function (thisContent, callback) {
      ...
      $('.lottery-details > .w-tabs').on('click', function () {
...
//                  console.log(recontent, 'recontent');
      ...
      success: function (list) {
        ...
        $.each(list, function (i, val) {
          ...
          var innerHtml =
            ...
            '<td class="repeatbtn" rel="' + val.methodCode
            + '" model="' + val.model + '" time="' + val.multiple + '" method="'
            + val.method + '" content="' + formatRepeatContent(val.content, val.method, val.methodCode) + '">'
            + (val.lottery == (GameData.getInfo() && GameData.getInfo().showName) ? actions : '') + '<a data-command="gendan">跟单</a></td>' +
            '</tr>';
  ...
  var refreshHistory = function () {
    if ($('.record-list .tabs .active').data('key') == 'NotOpenOrder') {
      initNotOpenOrder();
    }
    if ($('.record-list .tabs .active').data('key') == 'HistoryOrder') {
      initHistoryOrder();
    }
    if ($('.record-list .tabs .active').data('key') == 'ChaseOrder') {
      initChaseOrder();
    }
...
var Route = {
  ...
  LotteryBonusTip: function (id) {
    ...
    if (id == '911' || id == '50') {
      limitnum = Route.LotteryBonusGroups[1];
    }
...
var PankouData = function () {
  ...
  var quickchkpost = {};
  var quickmoney = 0;
  var cachedata1 = null, cachedata2, cachedata3, cachedata4;

  //{"id":822,"gname":"斗牛","BetContext":"牛5","Lines":"15.629","BetType":1,"Money":"5.00","IsTeMa":false,"IsForNumber":false,"mingxi_1":0}
  //初始化
  var init = function (id, code, callb) {
    var opt = {}

    opt.data = JSON.stringify({"lotteryId": code, "numberPostion": ""});
  ...
  //倒计时结束
  var reinit = function (id, code, callb) {
    var opt = {}
  ...
  var tabletab = function () {
    var initluzhu = function () {
      $('#luzhu .tab-item').off('click').on('click', function () {
        var nowindex = $(this).index();
        $('#luzhu .tab-item').removeClass('active');
        $(this).addClass('active');
    ...
    $('#wenlu .tab-item').first().addClass('active');
    $('#luzhu .tab-item').first().addClass('active');
    $('#wenlu .tab-item').off('click').on('click', function () {
      var nowindex = $(this).index();
      $('#wenlu .tab-item').removeClass('active');
      $(this).addClass('active');
      //console.log(nowindex+1);
      var realindex = nowindex + 1;
      //console.log(realindex,'realindexrealindexrealindex');
      rendertrend(realindex, cachedata1, cachedata2, cachedata3, cachedata4, function (a) {
        //var rendered = cachedata4.mustache.render(cachedata4.wenlutpl, cachedata4.wenlu);
        var rendered1 = cachedata4.mustache.render(cachedata4.luzhutpl, cachedata4.luzhu);
        //cachedata4.wenluhtml= rendered;
        //console.log(rendered1,'rendered1rendered1rendered1');
        $('#luzhu').html(rendered1);
        //cachedata4.luzhuhtml= rendered1;
        initluzhu();
        $('#luzhu .tab-item').first().addClass('active');
        $('#luzhu .tab-bd').first().addClass('tab-bd-in');
        //$('#wenlu .tab-item').first().addClass('active');
    ...
    $('.pankou-opearation .play-groups > ul li').off('click').on('click', function () {
      ...
      //console.log(thisrel,'thisrelthisrel');
      ...
      if ($('#j-all > [rel="' + thisrel + '"]').length == 0) {
        $('#j-pk > div').hide();
        $('#j-pk > #' + thisrel).addClass('now').show();
      }
      if (thisrel == 'j-n6' || thisrel == 'j-n7') {
        $('#j-lhc .nowm_panel .quickmoney').hide();
      } else {
        if ($('#j-n4 .zhengmapos li[rel="6"]').hasClass('on')) {
          $('#j-lhc .nowm_panel .quickmoney').hide();
        } else {
          $('#j-lhc .nowm_panel .quickmoney').show();
        }

      }
      if ($('#j-lhc:visible').size() > 0) {
        $('#j-lhc .now').removeClass('now').hide();
        $('#j-lhc > div:not(.nowm_panel)').hide();
        $('#j-lhc .quickfilter ul').hide();
        $('#j-lhc > #' + thisrel).addClass('now').show();
        $('#j-lhc .quickfilter ul.' + thisrel + '-filter').show();
      }
      $('.pankou-opearation .tac .parlay-reset').click();
    });
    initluzhu();
    ...
    $('.pankou-opearation .tac .parlay').off('click').on('click', function () {
      var lidcode = $('.pankou-opearation .tac .parlay').attr('rel');
      var formatParams = {"lotteryId": lidcode, "betParameters": []};
      var modestr = $('.pankou-opearation .button-secondary-group .button-current').data('mode');
      var letsee = [];
      var totalm = 0;
      var totaldeal = 0;
      var needgroup = false;
      var groupRes = _.chain(chkpost).groupBy("Money").map(function (v, i) {
        return {
          name: i,
          rname: _.get(_.find(v, 'gname'), 'gname'),
          rate: _.get(_.find(v, 'Lines'), 'Lines'),
          rid: _.get(_.find(v, 'id'), 'id'),
          bets: _.map(v, 'BetContext')
        }
      }).value();
      console.log(chkpost, groupRes, 'groupResgroupRes');
      if ($('#j-lhc .now').attr('id') == 'j-n3' || $('#j-lhc .now').attr('id') == 'j-n4' || $('#j-lhc .now').attr('id') == 'j-n5') {
        needgroup = false;
      }
      if (!needgroup) {
        for (key in chkpost) {
          ...
          totalm += Number(nowitm.Money);
          console.log(nowitm.rate, 'nowitm.ratenowitm.ratenowitm.rate');
          if (nowitm.gname != '六肖') {
            letsee.push('<li><div class="cftip totalm">' + nowitm.gname + '【' + nowitm.BetContext + '】@' + nowitm.rate + ' X ' + nowitm.Money + '</div></li>');
          } else {
            letsee.push('<li><div class="cftip totalm">' + nowitm.gname + '【' + nowitm.BetContext + '】 X ' + nowitm.Money + '</div></li>');
          }

        }
      } else {
        for (var i = 0; i < groupRes.length; i++) {
          formatParams["betParameters"].push({
            "id": groupRes[i].rid,
            "gname": groupRes[i].rname,
            "BetContext": '' + groupRes[i].bets.join('|') + '',
            "Lines": groupRes[i].rate,
            "BetType": 1,
            "Money": groupRes[i].name,
            "IsTeMa": false,
            "IsForNumber": false,
            "mingxi_1": 0
          });
          //console.log(chkpost[key]);
          var nowitm = groupRes[i];
          totalm += Number(groupRes[i].name);
          letsee.push('<li><div class="cftip totalm">' + nowitm.rname + '【' + groupRes[i].bets.join('|') + '】@' + groupRes[i].rate + ' X ' + groupRes[i].name + '</div></li>');
        }
      }


      //console.log(modestr,'modestrmodestrmodestr');
      if (modestr == 'quick') {
        letsee = [];
        for (key in quickchkpost) {
          formatParams["betParameters"].push(quickchkpost[key]);
          var nowitm = quickchkpost[key];
          nowitm.Money = quickmoney;
          totalm += Number(quickmoney);
          letsee.push('<li><div class="cftip totalm">' + nowitm.gname + '【' + nowitm.BetContext + '】@' + nowitm.Lines + ' X ' + nowitm.Money + '</div></li>');
        }
      }
      if ($('#j-lhc').size() > 0) {
        //letsee.push('<li><div class="cftip totalm">六合彩【测试】@11.47 ￥10.2</div></li>');
        //alert('111');
        //[{"id":749,"gname":"第二球","BetContext":"大","Lines":"1.960","BetType":1,"Money":"2.00","IsTeMa":false,"IsForNumber":false,"mingxi_1":0},{"id":750,"gname":"第二球","BetContext":"小","Lines":"1.960","BetType":1,"Money":"2.00","IsTeMa":false,"IsForNumber":false,"mingxi_1":0}]
        //formatParams["betParameters"].push({"id":"lhc_tema","gname":"特码","BetContext":"(01|02|03|04|05)","Lines":"12.01","BetType":1,"Money":"1.00","IsTeMa":false,"IsForNumber":false,"mingxi_1":0});
      }
      if (letsee.length == 0) {
        App.alert('success', '提示消息', '请选择一注号码', 3000);
        return false;
      }

      if (letsee.length > 0 && parseInt(totalm, 10) <= 0) {
        App.alert('success', '提示消息', '投注额有误', 3000);
        return false;
      }

      if (parseInt($('.pankou-opearation .now #totalcount em').first().html(), 10) == 0) {
        App.alert('success', '提示消息', '请正确选择号码', 3000);
        return false;
      }

      //$('.pankou-opearation .now #totalcount em').first().html(letsee.length);
      //$('.pankou-opearation .now #totalcount em').last().html(Number(totalm).toFixed(2));
      ...
      BootstrapDialog.show({
        ...
        message: function () {
          return [
            ...
            "<div class='cftip totalm'>共计：￥" + $('.pankou-opearation .now #totalcount em').last().html() + "/" + $('.pankou-opearation .now #totalcount em').first().html() + "注，您确定要下注吗？</div><ul class='limithp'>",
            letsee.join(''),
    ...
    $('.pankou-opearation .tac .parlay-reset').off('click').on('click', function () {
      //console.log('reset');
      $('.pankou-opearation .now .j-odds input').val('');
      $('.pankou-opearation #j-all .j-betting .table-current').removeClass('table-current');
      $('.pankou-opearation .tac .sync-bet').val('');
      $('.pankou-opearation .now .nowm_list .choosed').removeClass('choosed');
      $('.statistic .num_i').html('0');
      $('#j-lhc .lhc_uls input').val('');
      $('.pankou-opearation .now #totalcount em').first().html("0");
      $('.pankou-opearation .now #totalcount em').last().html(Number(0).toFixed(2));
      chkpost = {};
      quickchkpost = {};
      $('.statistic .money_i').html('0');
    });
    //快捷切换
    $('.pankou-opearation .button-secondary-group button').off('click').on('click', function () {
      ...
      if (thismode == 'quick') {
        ...
        $('.pankou-opearation .tac_money').show();
      } else {
        $('.pankou-opearation #j-all .j-betting .table-current').removeClass('table-current');
        $('#j-all .j-betting td.j-odds').show();
        $('.pankou-opearation .tac_money').hide();
        $('.statistic .num_i').html('0');
        chkpost = {};
        quickchkpost = {};
        $('.statistic .money_i').html('0');
    ...
    //快捷的赔率事件
    $('.pankou-opearation #j-all .j-betting > tbody').off('click').on('click', 'tr td', function () {
      var modestr = $('.pankou-opearation .button-secondary-group .button-current').data('mode');
      var ptr = $(this).parent();
      var betNum = 0, betMoney = 0;
      if (modestr == 'quick' && !$(this).hasClass('td-hd')) {

        var nowtd = $(this).parent();
        var hasame = nowtd.hasClass('same-id');
        ...
        var nowDeal = {
          ...
          "Lines": lnrate,
          "BetType": 1,
          "Money": Number(quickmoney).toFixed(2),
          "IsTeMa": false,
          "IsForNumber": false,
          "mingxi_1": 0
        };
        ...
        if (hasame) {
          tdid = nowtd.data('id') + '_' + lnhead;
        }
        //console.log(nowDeal);
        if (!ptr.hasClass('table-current')) {
          ptr.addClass('table-current');
          quickchkpost[tdid] = nowDeal;
        } else {
          ptr.removeClass('table-current');
          delete quickchkpost[tdid];
        ...
        //console.log(quickchkpost);
        for (var key in quickchkpost) {
          //console.log(!isNaN(quickchkpost[key].Money - 0));
          if (!isNaN(quickchkpost[key].Money - 0)) {
            betNum += 1;
            //console.log(parseInt(quickchkpost[key].Money,10));
            if (parseInt(quickchkpost[key].Money, 10) == 0 || parseInt(quickchkpost[key].Money, 10) != parseInt($('.pankou-opearation .tac .sync-bet').val(), 10)) {
              betMoney += parseInt($('.pankou-opearation .tac .sync-bet').val(), 10) - 0;
            } else {
              betMoney += quickchkpost[key].Money - 0;
            }
        ...
        //console.log(betNum,betMoney,'betMoneybetMoneybetMoney');
        $('.statistic .num_i').html(betNum);
        $('.statistic .money_i').html(betMoney);
    ...
    //六合彩快速选择
    $('#j-lhc .colorfilter').off('click').on('click', 'li', function () {
      //console.log($(this).attr('rel'));
      $('#j-lhc .sxfilter li').removeClass('choosed');
      $('#j-lhc .quickmoney .quickset a').removeClass('on');
      var allindex = $(this).attr('rel').split(',');
      var fullsel = '#j-lhc .now .ballno_' + allindex.join(',#j-lhc .now .ballno_');
      if (String($(this).attr('rel')).indexOf('hong') > -1 || String($(this).attr('rel')).indexOf('lv') > -1 || String($(this).attr('rel')).indexOf('lan') > -1) {
        fullsel = '#j-lhc .now .textno_' + allindex.join(',#j-lhc .now .textno_');
      }

      if (String($(this).attr('typ')) == 'text') {
        fullsel = '#j-lhc .now .textno_' + allindex.join(',#j-lhc .now .textno_');
      }
      $('#j-lhc .now .highlight').removeClass('highlight');
      //console.log(fullsel);
      if (!$(this).hasClass('choosed')) {
        $('#j-lhc .colorfilter li').removeClass('choosed');
        $(this).addClass('choosed');
        $(fullsel).addClass('highlight');
      } else {
        //console.log('aaaaa');
        $('#j-lhc .colorfilter li').removeClass('choosed');
        $('#j-lhc .now .highlight').removeClass('highlight');
    ...
    $('#j-lhc .sxfilter').off('click').on('click', 'li', function () {
      //console.log($(this).attr('rel'));
      var allindex = $(this).attr('rel').split(',');
      var fullsel = '#j-lhc .now .ballno_' + allindex.join(',#j-lhc .now .ballno_');
      if (String($(this).attr('rel')).indexOf('hong') > -1 || String($(this).attr('rel')).indexOf('lv') > -1 || String($(this).attr('rel')).indexOf('lan') > -1) {
    ...
    $('#j-n2 .zhengmapos ul').off('click').on('click', 'li', function () {
      $('#j-n2 .zhengmapos ul li').removeClass('on');
      $(this).addClass('on');
      $('#j-lhc .lhc_uls input').val('');
      var nowhelp = $(this).attr('title');
      $('#j-n2 .nowm_help span').html(nowhelp);
      chkpost = {};
      quickchkpost = {};
      $('.pankou-opearation .now #totalcount em').first().html("0");
      $('.pankou-opearation .now #totalcount em').last().html(Number(0).toFixed(2));
    ...
    $('#j-n4 .zhengmapos ul').off('click').on('click', 'li', function () {
      $('#j-n4 .zhengmapos ul li').removeClass('on');
      $(this).addClass('on');
      $('#j-lhc .lhc_uls input').val('');
      chkpost = {};
      quickchkpost = {};
      ...
      var nowrate = $(this).attr('rate').split('|')[0].replace(/([\d])+@/g, "");
      var nowrate1 = $(this).attr('thisr').split('|')[0].replace(/([\d])+@/g, "");
      $('#j-n4 .nowm_help span').html(nowhelp);
      /*if (thisrel=='j-n6' || thisrel=='j-n7') {
            $('#j-lhc .nowm_panel .quickmoney').hide();
          }else {
            $('#j-lhc .nowm_panel .quickmoney').show();
      ...
      if ($(this).attr('rel') == '6') {
        $('#j-lhc #j-n4 .lineinput .inline-right > input').hide();
        $('#j-lhc #j-n4 .lineinput .inline-right > span.chkdot').show();
        $('#liuxiaom,.now_samelb').show();
        $('#j-lhc #j-n4 .lineinput .inline-left > .slabel').hide();
        $('#j-lhc #j-n4 .nowm_rate').show();
        $('#j-lhc .nowm_panel .quickmoney').hide();
      } else {
        var titledict = {'0': '特肖', '1': '一肖'};
        var titledictinput = {'0': 'lhc_texiao_', '1': 'lhc_yixiao_'};
        $('#j-lhc #j-n4 .lineinput .inline-left > .slabel').show();
        $('#liuxiaom,.now_samelb').hide();
        $('#j-lhc #j-n4 .nowm_rate').hide();
        $('#j-lhc #j-n4 .lineinput .inline-right > input').map(function (k, el) {
          //console.log(k,el);
          var oldrel = $(el).attr('rel').split('_')[2];
          $(el).attr('rel', titledictinput[nowrel] + oldrel);
        });
        $('#j-lhc .nowm_panel .quickmoney').show();
        //console.log(nowrate);
        $('#j-lhc #j-n4 .nowm_list li.lineinput').attr('title', titledict[nowrel]);
        $('#j-lhc #j-n4 .lineinput .inline-right > input').show();
        $('#j-lhc #j-n4 .lineinput .inline-right > span.chkdot').hide();
      }

      $('.pankou-opearation .now #totalcount em').first().html("0");
      $('.pankou-opearation .now #totalcount em').last().html(Number(0).toFixed(2));
      $('.pankou-opearation .play-fire [data-action="reset"]').click();
      $('#j-lhc #j-n4 .lineinput:not(.nowxiao) .inline-left > .slabel').html('x' + nowrate);
      $('#j-lhc #j-n4 .nowxiao .inline-left > .slabel').html('x' + nowrate1);
    });

    $('#j-n7 .zhengmapos ul').off('click').on('click', 'li', function () {
      $('#j-n7 .zhengmapos ul li').removeClass('on');
      $(this).addClass('on');
      var nowrel = $(this).attr('rel');
      var nowhelp = $(this).attr('title');
      var nowrate = $(this).attr('rate').split('|')[0].replace(/([\d])+@/g, "");
      $('#j-lhc .lhc_uls input').val('');
      $('#j-n7 .nowm_help span').html(nowhelp);
      $('#j-lhc #j-n7 .lineinput .inline-left > .slabel').html('x' + nowrate);
      var titledict = {'0': '五不中', '1': '六不中', '2': '七不中', '3': '八不中', '4': '九不中', '5': '十不中'};
      var titledictinput = {
        '0': 'lhc_buzhong_wu',
        '1': 'lhc_buzhong_liu',
        '2': 'lhc_buzhong_qi',
      ...
      $('#bzrate').html(nowrate);
      $('#j-lhc #j-n7 .lineinput').map(function (k, el) {
        //console.log(k,el);
        //var oldrel = $(el).attr('rel').split('_')[2];
        $(el).attr('rate', nowrate);
      });
      $('.pankou-opearation .now #totalcount em').first().html("0");
      $('.pankou-opearation .now #totalcount em').last().html(Number(0).toFixed(2));
      //console.log(nowrate);
      $('#j-lhc #j-n7 .nowm_list li.lineinput').attr('rel', titledict[nowrel]);
      $('.pankou-opearation .play-fire [data-action="reset"]').click();
      $('.pankou-opearation .now .nowm_list .choosed').removeClass('choosed');
      chkpost = {};
      quickchkpost = {};
    ...
    $('#j-lhc .quickmoney .quickset').off('click').on('click', 'a', function () {
      $('#j-lhc .quickmoney .quickset a').removeClass('on');
      $(this).addClass('on');
      $('.pankou-opearation .now .highlight input').val($(this).text());
      $('.pankou-opearation .lhc_uls.now input').blur();
    });
  }
  //盘口事件
  var reCount = function () {
    ...
    //console.log(betNum,betMoney,'11111betMoneybetMoneybetMoney');
    $('.statistic .num_i').html(betNum);
    $('.statistic .money_i').html(betMoney);
  }

  var pankoucountdeal = function (chk) {
    var inletsee = [], inmoney = 0;
    for (key in chk) {
      var nowitm = chk[key];
      inmoney += Number(nowitm.Money);
      inletsee.push('1');
    }
    $('.pankou-opearation .now #totalcount em').first().html(inletsee.length);
    $('.pankou-opearation .now #totalcount em').last().html(Number(inmoney).toFixed(2));
  }
  ...
  var pankouevent = function () {
    ...
    $('.j-betting .j-odds input').on('blur', function () {
      ...
      if (!isNaN(tdm - 0)) {
        ...
        //console.log(chkpost,tdid,lnhead);
        if (typeof chkpost[tdid] == 'undefined') {
          chkpost[tdid] = nowDeal;
        ...
        // 盘口失去焦点事件
        for (var key in chkpost) {
          if (!isNaN(chkpost[key].Money - 0)) {
            betNum += 1;
            betMoney += chkpost[key].Money - 0;
          }
        }
        $('.statistic .num_i').html(betNum);
        $('.statistic .money_i').html(betMoney);
    ...
    $('.lhc_uls input').on('blur', function () {
      //console.log($(this).val());
      var nowval = $(this).val();
      $(this).val(nowval.replace(/[^0-9]/ig, ""))

      if ($(this).attr('id') == 'liuxiaom') {
        return false;
      }
      if ($(this).attr('id') == 'liuxiaom_1') {
        return false;
      }

      var nowin = $(this);
      var nowtd = $(this).parent().parent();
      var tdid = nowin.attr('rel');
      var hasame = nowin.parent().hasClass('same-id');
      var tdm = $(this).val();
      var tdhead = nowin.parent().attr('rel');
      var lnhead = nowin.parent().attr('rid');
      var lnrate = nowin.parent().attr('rate');
      var lnrate_old = nowin.parent().attr('rate');
      if (typeof nowin.parent().attr('bonus') != 'undefined') {
        lnrate = nowin.parent().attr('bonus');
      }
      var betNum = 0; // 投注数
      var betMoney = 0; // 投注额

      if (String(tdid).indexOf('zhengma') > -1) {
        tdid = 'lhc_zhengma' + $('#j-n2 .zhengmapos li.on').attr('rel');
        tdhead = tdhead + $('#j-n2 .zhengmapos li.on').attr('rel');
      }
      console.log(typeof lnrate == 'undefined');


      var formatBetContext = function (tdid, str) {
        if (String(tdid).indexOf('tema') > -1 || String(tdid).indexOf('zhengma') > -1 || String(tdid).indexOf('buzhong') > -1) {
          return '(' + str + ')';
        }
        if (String(tdid).indexOf('lhc_liuxiao') > -1) {
          return '(' + str + ')';
        }
        return str;
      }

      if (typeof lnhead == 'undefined' && nowtd.find('.flabel').size() > 0) {
        lnhead = nowtd.find('.flabel').text();
        tdhead = nowtd.attr('title');
        if (String(tdhead).indexOf('特肖') > -1 || String(tdhead).indexOf('一肖') > -1) {
          lnhead = nowtd.attr('title') + nowtd.find('.flabel').text();
          tdhead = nowtd.attr('title') + nowtd.find('.flabel').text();
        }
        if (String(tdhead).indexOf('总分') > -1) {
          lnhead = nowtd.attr('title') + nowtd.find('.flabel').text();
          tdhead = nowtd.attr('title') + nowtd.find('.flabel').text();
        }
        if (String(tdhead).indexOf('尾数') > -1) {
          tdhead = nowtd.attr('title') + String(nowtd.find('.flabel').text()).replace('尾', '');
          lnhead = String(nowtd.find('.flabel').text()).replace('尾', '');
        }
        //lnrate = String(nowtd.find('.slabel').text()).replace('x','');
        lnrate = nowtd.find('.slabel').attr('bonus');
        lnrate_old = String(nowtd.find('.slabel').text()).replace('x', '');
        console.log('11111', tdid, lnrate, lnrate_old);
      }

      //if (typeof tdhead =='undefined') {
      //
      //}
      /*if (typeof lnrate =='undefined') {
              lnrate = nowtd.find('.slabel').attr('bonus');
              alert(nowtd.find('.slabel').attr('bonus'));
              lnrate_old = String(nowtd.find('.slabel').text()).replace('x','');
            }*/
      tdid = String(tdid).replace('_weishu_w', '_weishu_');
      console.log(tdid, tdhead, lnhead, lnrate, tdm);
      // {"id":"lhc_tema","gname":"特码","BetContext":"(01|02|03|04|05)","Lines":"12.01","BetType":1,"Money":"1.00","IsTeMa":false,"IsForNumber":false,"mingxi_1":0}
      if (!isNaN(tdm - 0)) {
        var nowDeal = {
          "id": lhc_idmaps[tdid],
          "gname": tdhead,
        ...
        if (hasame) {
          if (String(tdid).indexOf('zhengma') > -1) {
            tdid = 'lhc_zhengma' + $('#j-n2 .zhengmapos li.on').attr('rel') + '_' + lnhead;
          } else {
            tdid = nowin.attr('rel') + '_' + lnhead;
          }
        }
        //console.log(tdid,'tdidtdidtdid');
        if (typeof chkpost[tdid] == 'undefined') {
          chkpost[tdid] = nowDeal;
          //postparams["betParameters"].push();
        } else {
          chkpost[tdid] = nowDeal;
          //var nowBetContext = chkpost[tdid]['BetContext'];
          //if (String(nowBetContext).indexOf("|")==-1) {
          //chkpost[tdid]['BetContext'] = nowBetContext+'|'+lnhead;
          //}else {
          //}
      ...
      console.log(chkpost, 'chkpostchkpostchkpostchkpostchkpostchkpost');
      pankoucountdeal(chkpost);
    });

    $('#j-lhc #j-n7 .chkdot').off('click').on('click', function () {
      var thisli = $(this).parents('li');
      var tdid = $('#j-lhc #j-n7 .zhengmapos .on').attr('nid');
      var tdlen = $('#j-lhc #j-n7 .zhengmapos .on').attr('len');
      if ($('#liuxiaom_1').val() == '') {
        $('#liuxiaom_1').val("1");
      }
      var allrid;
      if (!thisli.hasClass('choosed')) {
        thisli.addClass('choosed');
        allrid = $('#j-lhc #j-n7 .choosed').map(function () {
          return $(this).attr('rid');
        }).get();
        chkpost[tdid] = {
          "id": tdid,
          "gname": thisli.attr('rel'),
          "BetContext": allrid.join(','),
          "Lines": '1960',
          "rate": thisli.attr('rate'),
          "BetType": 1,
          "Money": '1.00',
          "IsTeMa": false,
          "IsForNumber": false,
        ...
        thisli.removeClass('choosed');
        allrid = $('#j-lhc #j-n7 .choosed').map(function () {
          return $(this).attr('rid');
        }).get();
        chkpost[tdid]["BetContext"] = allrid.join(',');
      }
      console.log(allrid);
      if (allrid.length > (parseInt(tdlen, 10) - 1)) {
        var cmb = Combinatorics.combination(allrid, parseInt(tdlen, 10));
        // cmb.length;
        console.log(cmb.length);
        $('.pankou-opearation .now #totalcount em').first().html(cmb.length);
        $('.pankou-opearation .now #totalcount em').last().html(Number(parseInt($('#liuxiaom_1').val(), 10) * cmb.length).toFixed(2));
      } else {
        $('.pankou-opearation .now #totalcount em').first().html("0");
        $('.pankou-opearation .now #totalcount em').last().html(Number(0).toFixed(2));
      }
      ...
      if ($('#j-lhc #j-n7 .choosed').size() == 0) {
        delete chkpost[tdid];
      }
    });

    $('#j-lhc #j-n4 .chkdot').off('click').on('click', function () {
      var thisli = $(this).parents('li');
      var tdid = lhc_idmaps['lhc_liuxiao'];
      var tdlen = $('#j-lhc #j-n4 .zhengmapos .on').attr('len');
      if ($('#liuxiaom').val() == '') {
        $('#liuxiaom').val("1");
      }
      var allrid;
      console.log(chkpost);
      //console.log(allrid,'allridallrid');
      if (!thisli.hasClass('choosed')) {
        thisli.addClass('choosed');
        allrid = $('#j-lhc #j-n4 .choosed').map(function () {
          return $(this).find('.flabel').text();
        }).get();
        chkpost[tdid] = {
          "id": tdid,
          "gname": '六肖',
          ...
          "BetType": 1,
          "rate": "1.96",
          "Money": Number($('#liuxiaom').val()).toFixed(2),
          "IsTeMa": false,
          "IsForNumber": false,
          "mingxi_1": 0
        };
      ...
      console.log(allrid.length > (parseInt(tdlen, 10) - 1) && typeof tdlen != 'undefined', 'allrid.length>(parseInt(tdlen,10)-1) && typeof tdlen !=');
      if (allrid.length > (parseInt(tdlen, 10) - 1) && typeof tdlen != 'undefined') {
        var cmb = Combinatorics.combination(allrid, parseInt(tdlen, 10));
        // cmb.length;
        console.log(cmb.length);
        $('.pankou-opearation .now #totalcount em').first().html(cmb.length);
        $('.pankou-opearation .now #totalcount em').last().html(Number(parseInt($('#liuxiaom').val(), 10) * cmb.length).toFixed(2));
      } else {
        $('.pankou-opearation .now #totalcount em').first().html("0");
        $('.pankou-opearation .now #totalcount em').last().html(Number(0).toFixed(2));
      }

      if ($('#j-lhc #j-n4 .choosed').size() == 0) {
        delete chkpost[tdid];
      }
      console.log(chkpost, 'chkpostchkpostchkpost');
    });

    $('#liuxiaom').on('blur', function () {
      var tdlen = $('#j-lhc #j-n4 .zhengmapos .on').attr('len');
      var allrid = $('#j-lhc #j-n4 .choosed').map(function () {
        return $(this).find('.flabel').text();
      }).get();
      console.log(chkpost);
      chkpost[25049]["Money"] = Number($('#liuxiaom').val()).toFixed(2);
      if (allrid.length > (parseInt(tdlen, 10) - 1) && typeof tdlen != 'undefined') {
        var cmb = Combinatorics.combination(allrid, parseInt(tdlen, 10));
        // cmb.length;
        //console.log(cmb.length);
    ...
    $('#liuxiaom_1').on('blur', function () {
      var allrid = $('#j-lhc #j-n7 .choosed').map(function () {
        return $(this).find('.flabel').text();
      }).get();
      var tdid = $('#j-lhc #j-n7 .zhengmapos .on').attr('nid');
      var tdlen = $('#j-lhc #j-n7 .zhengmapos .on').attr('len');
      chkpost[tdid]["Money"] = Number($('#liuxiaom_1').val()).toFixed(2);
      if (allrid.length > (parseInt(tdlen, 10) - 1) && typeof tdlen != 'undefined') {
        var cmb = Combinatorics.combination(allrid, parseInt(tdlen, 10));
        // cmb.length;
        //console.log(cmb.length);
    ...
    $('.pankou-opearation .tac .sync-bet').on('focus', function () {
      reCount();
    });

    $('.pankou-opearation .tac .sync-bet').on('blur', function () {
      reCount();
    });

    $('.pankou-opearation .tac_money input').on('focus', function () {
      quickmoney = parseInt($(this).val(), 10);
      //console.log(quickmoney, 'focusfocusfocusfocus');
    });
    $('.pankou-opearation .tac_money input').on('blur', function () {
      quickmoney = parseInt($(this).val(), 10);
      //console.log(quickmoney, 'blurblurblurblurblurblur');
    });

    $('.now_samefire').off('click').on('click', function () {
      $('.pankou-opearation .play-fire [data-action="parlay"]').click();
    });

    $('.now_samereset').off('click').on('click', function () {
      $('.pankou-opearation .play-fire [data-action="reset"]').click();
    });
  }
  //获取开奖号码
  ...
  var firehole = function (data) {
    ...
    opt.success = function (response) {
      ...
      if (response.error == '1') {
        App.alert('success', '提示消息', response.message, 3000);
      }
      if (response.result == '1') {
        ...
        quickchkpost = {};
        $('.pankou-opearation #j-all .j-betting .table-current').removeClass('table-current');
        $('.pankou-opearation .tac_money input').val('');
  ...
  //问路和路珠
  var rendertrend = function (kindex, data, data2, lz, t, back) {
    // console.log(data.slice(0, 5),'rendertrend');
    ...
    t.wenlu.bd = od;
    t.luzhu = endu;
    //console.log(o,od,endu,'t.wenlut.wenlut.wenlut.wenlut.wenlu');
  ...
  function changeTabs(initPages, callback) {
    ...
    manager.each(function (idx, ele) {
      if (!$(ele).hasClass('fixed') && !$(ele).hasClass('disabled')) {
        if (typeof $(ele).attr('m') == 'undefined') {
          $(ele).attr('m', idx);
        }
...
var formatSeconds = function (value, k) {
  ...
  //console.log(theTime,theTime1,theTime2);
  if (k == 99) {
    result = '<i>' + result.replace(/:/g, '</i><em class="dots">:</em><i>') + '</i>';
  }
  return {
    'str': result,
    'k': k
...
var tick = function (a) {
  ...
  var objss = $('.lottery-open-list-pankou .list div').eq(0).find('.code');
  //console.log(_ISONLYINIT);
  if ($('#pankou-timer').size() > 0) {
    var pleftsec = parseInt($('#pankou-timer').data('left'), 10);
    pleftsec--;
    if (pleftsec <= 0 && _ISONLYINIT) {
      _ISONLYINIT = false;
      var panVue = appConfig().pankouInit();
      PankouData.reinit(parseInt($('#hashlid').text(), 10), $('#hashlid').attr('rel'), function (a) {
        if (typeof a.Obj != 'undefined') {
          var allinfo = a.Obj;
          pleftsec = allinfo.CloseCount;
        }
      });
      panVue.initLotteryPage();
    }
    //console.log(objss.html())
    if (pleftsec % 10 == 0 && objss.html() == '开奖中') {
      var panVue = appConfig().pankouInit();
      panVue.initLotteryPage();
    }
    _ISONLYINIT = true;
  ...
  LoopManager.loopmanage_3 = window.setTimeout("tick(1)", 950);
}
tick(0);
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.error == 0) {
            ...
            if (RecordList) {
              RecordList.init();
//                          console.log('refreshHistoryrefreshHistory');
...
var LotteryOpenTime = function () {
  ...
  var setTime = function (issue, openTime) {
    ...
    if (seconds == 0) {
      ...
      if ($('#sys-message').size() > 0) {
        ...
        if ($.cookie('WIN-VOICE') === 'ON' && !isMobile.any()) {
          var audio = $('<audio id="sys-message" autoplay="autoplay">');
          audio.attr('src', '/static/v3/sound/message.mp3').hide();
          $('body').append(audio);
          //document.getElementById('sys-message').play();
        }
      }
    ...
    if (seconds % 8 == 0 && (GameData.getInfo().id == '61' || GameData.getInfo().id == '50' || GameData.getInfo().id == '911')) {
      typeof RecordList != 'undefined' && RecordList.init();
      //console.log('LotteryOpenCode.init');
      var linfo = GameData.getInfo();
      LotteryOpenCode.init(linfo);
    }
    if (seconds % 5 == 0 && (GameData.getInfo().id == '46')) {
      typeof RecordList != 'undefined' && RecordList.init();
      //console.log('LotteryOpenCode.init');
      var linfo = GameData.getInfo();
      LotteryOpenCode.init(linfo);
    }
    if (seconds > 30 && seconds < 50 && (GameData.getInfo().id != '61' && GameData.getInfo().id != '50' && GameData.getInfo().id != '911')) {
    ...
    if (seconds <= 0) {
      if (typeof LoopManager.loopmanage_1 != 'undefined') {
        clearInterval(LoopManager.loopmanage_1);
      }
      ...
      //console.log(LoopManager.loopmanage_1,'LoopManager.loopmanage_1LoopManager.loopmanage_1');
      LoopManager.loopmanage_1 = $Timer;
    }
  }
  ...
  var handleData = function (data) {
    ...
    LoopManager.loopmanage_2 = $Timer;
  }
...
var LotteryOpenCode = function () {
  ...
  var handleData = function (list) {
    ...
    $.each(list, function (i, val) {
      ...
      item.append('<div class="expect">' + val.issue + '期</div>');
  ...
  var showOpenCode = function (data) {
    ...
    if (data.code != null) {
      ...
      $('#lottery .lastopis,#basket .lastopis,#zuomainbox .lastopis').html((String(data.issue).indexOf('-') == -1 ? String(data.issue).substring(data.issue.length - 3, data.issue.length) : String(data.issue).split('-')[1]));
      $('div[data-field="code"]').hide();
      //console.log('aaaaa111',data.issue,data);
      $('.codeholder').show();
    }
  }
...
var setLogo = function () {
  ...
  var locationUrl = location.hash.substr(1);
  var officialUrl = appConfig().officialUrl;
  var url = ''; // 默认彩票官网地址
  for (var k in officialUrl) {
    locationUrl + '' === k && (url = officialUrl[k]);
  }
  // var wrapper1 = $('<div class="wrapper"><div class="logo animated flash"></div><div class="nav">' +
  //     '<a href="#" class="lottery-code-trend" target="_blank"><em class="icon">&#xe69e;</em>走势图</a>' +
  //     '<a href="' + url + '" target="_blank"><em class="icon">&#xe68d;</em>彩票官网</a></div></div>');
  var wrapper2 = $('.lottery-open .wrapper');
  wrapper2.find('.official_url').attr('href', url);
  // lot.append(wrapper1);
...
//  $('.lottery-open').find('.logo').addClass(shortName.toLowerCase().replace('30', '2').replace(/[0346789]/g, ""));
  $('.lottery-open').find('.logo').removeClass().addClass('logo').addClass(shortName.toLowerCase().replace('30', '2').replace(/[0346789]/g, ""));
