...
var PopRechargeToUser = function() {
	...
	var buildRechargeUser = function(data, callback) {
		if (data) {
			Will.initBox('<i class="icon lock"></i>充值</span>',initDoc(data),600,340,function(){
				initEvent(callback);
			});
 		}
	}
  ...
  ////console.log(thisContent02,'thisContent02thisContent02');
    ...
    if (data==null && AppData.getMainAccount().type==0) {
      window.location.href = "/yx/hbs/manager-account.html#page=0";
		...
		// type.find('input[name="type"]').unbind('click').click(function() {
    ...
    type.find('.groupDivEventtype').unbind('click').click(function() {
      ...
      if (maxPoint==12.8 && parseInt(AppData.getMainAccount().userLevel,10)<5) {
        //addLinkPanel.find('[data-help="point"]').html('开户区间：'+ Number(maxPoint-0.1).toFixed(1));
      ...
      /*if (maxPoint==12.7) {
        //addLinkPanel.find('[data-help="point"]').html('开户区间：'+ Number(maxPoint-0.1).toFixed(1));
        addLinkPanel.find('[data-help="point"]').html('');
        pointinput.val(Number(maxPoint-0.1).toFixed(1)).attr('disabled','disabled');
        addLinkPanel.find('input[name="point"]').val(Number(maxPoint-0.1).toFixed(1)).attr('disabled','disabled');
        //help.html('开户区间：' +  Number(maxPoint-0.1).toFixed(1));
        help.html('')
      }
      if (maxPoint==12.6) {
  ...
  var loadLinks = function() {
	...
	var doDelRegistLink = function(id) {
		Will.ajax({id: id},Route.PATH + '/agent/delete-regist-link', function(data){
			App.alert('success', '提示消息', '该注册衔接已成功删除！', 3000);
...
var initThisPage02_proxy = function() {
	...
	Will.page(thisContent03,getSearchParams,Route.PATH + '/agent/list-team-account','没有团队成员信息',function(list){
      ...
      var showsubbtn = true;
        ...
        actions += '<a data-command="salary" rel="'+val.userId+'"  class="colorLys1">工资管理</a><a data-command="dividend" rel="'+val.userId+'"  class="colorLys1">分红</a>';
        ...
        if (getSearchParams().subName !='') {
          actions = '<a data-command="recharge">充值</a>';
          showsubbtn=false;
        }
				var innerHtml =
	...
	Will.page(thisContent04,{},Route.PATH + '/agent/list-online-account','没有在线会员信息',function(list){
      ...
      $.each(list, function(i, val) {
				...
				'<tr>'+
					...
					'<td><a href="#page=4&username=' + val.username + '">订单</a><a href="#page=5&username=' + val.username + '">账单</a></td>'+
				'</tr>';
				thisResultTable.find('tbody').append(innerHtml);
			});
...
var initThisPage04_proxy = function() {
  ...
  params.find('input[name="submit"]').attr('url','/game-lottery/search-order');
	Will.page(thisContent05,getSearchParams,params.find('input[name="submit"]'),'没有查询到相关记录',function(list){
			thisResultTable.find('tbody').empty();
      var innerHtml='';
...
var initThisPage06_proxy = function() {
  ...
  var initBounsFirst = function() {
    ...
    Will.ajax({},Route.PATH + '/bonus/zj-bonus', function(zdata){
      ...
      if (parseFloat(zdata.yetDistributBonus)>0) {
