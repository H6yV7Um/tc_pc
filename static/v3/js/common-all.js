...
var UserSysMessage = function() {
    ...
    var cleanHtml = function(str) {
        var regx = /<[^>]*>|<\/[^>]*>/gm;
        var msgtxt = $("<span />", { html: str }).text();
        //console.log(msgtxt);
        msgtxt = msgtxt.replace(/&lt;/g,"<").replace(/&gt;/g,">");
        msgtxt = msgtxt.replace(regx,"");
        return msgtxt;
    }
    ...
    Api.getCommon('onlineLotterys', { t: new Date().getTime() }, function(res) {
            ...
            for (let z = 0; z < gameClassifyArr.length; z++) {
                ...
                let wrapper1 = '<ul class="title_ul">' +
                    ...
                    '<h1 class="title header_lottery' + tem.idz + '"></h1>' +
                    ...
                    for (let x = 0; x < obj.length; x++) {
                        if (obj[x].id!='250') {
                          innerWrapper.push(
                              ...
                              ((obj[x].id === 61 || obj[x].id === 50) ? '<span class="icon lottery_list_newAdd">&#xe60d;</span>' : '')  +
                              obj[x].showName +
                              '</a></li>');  
                        }else {
                          innerWrapper.push(
...
// 腾讯30秒和QQ分分彩增加开奖官网
function lotteryUrlShow () {
    let hash = window.location.hash;
    let dom = $('div.fixed-right a.open_lottery_link');
    if (hash == '#61') {
        dom.css('display','block').attr('href',appConfig().officalsite['61']);
    } else if (hash == '#50') {
        dom.css('display','block').attr('href',appConfig().officalsite['50']);
    } else {
