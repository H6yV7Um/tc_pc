...
var PopMessageDetail = function(){
							...
							'<tr class="actions">'+
								...
								'<td class="value">'+
									'<div class="button-groups">'+
										formatActions +
										'<input data-id="' + data.id + '" name="delete" type="button" class="button" value="删除"/>'+'<a href="/userMessage#page=1_message" ref="/yx/hbs/manager-message.html?replay=1&nick='+formatUserValue+'#page=1" onclick="$(\'.jBox-closeButton\').click()"><input data-name="' + formatUserValue + '" name="replay" type="button" class="button" value="回复"/></a>'+
	...
	var details = function(type, data, callback) {
	    console.error(initDoc(type,data,callback))
